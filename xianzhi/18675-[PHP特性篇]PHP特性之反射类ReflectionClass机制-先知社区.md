# [PHP特性篇]PHP特性之反射类ReflectionClass机制-先知社区

> **来源**: https://xz.aliyun.com/news/18675  
> **文章ID**: 18675
...
1. **避免动态调用危险函数**：禁止通过反射调用`exec`、`system`、`shell_exec`等命令执行函数，以及`eval`、`assert`等代码执行函数。
2. **谨慎使用**`setAccessible`：除非必要，否则不使用`setAccessible(true)`绕过访问控制，尤其避免对包含敏感操作的私有方法使用。
3. **限制反射范围**：在框架或库中，反射应仅用于已知的、可信的类和方法，避免对用户可控的未知类进行反射操作。
4. **开启 PHP 安全配置**：禁用危险函数（`disable_functions`）、限制 XML 外部实体（`libxml_disable_entity_loader(true)`）等，降低攻击成功概率
