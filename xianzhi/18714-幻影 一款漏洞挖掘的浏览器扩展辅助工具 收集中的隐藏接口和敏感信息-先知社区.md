# 幻影|一款漏洞挖掘的浏览器扩展辅助工具|收集中的隐藏接口和敏感信息-先知社区

> **来源**: https://xz.aliyun.com/news/18714  
> **文章ID**: 18714
...
* 一键基础扫描：自动提取页面内的 API、URL、域名、邮箱、手机号、路径、参数、注释、多类 Token/Key 等  
  ![](images/20250828105219-03b20e5c-83ba-1.png)
* 深度递归扫描：多层链接/资源爬取，支持并发、超时配置，并在新窗口中运行，不阻塞当前操作  
  ![](images/20250828105220-04115058-83ba-1.jpeg)  
  ![](images/20250828105221-047fe930-83ba-1.jpeg)
* 批量 API 测试：对扫描到的分类条目进行 GET/POST 批测，并发与超时可配置，结果支持复制  
  ![](images/20250828105221-04d2cc36-83ba-1.jpeg)  
  ![](images/20250828105222-05382126-83ba-1.jpeg)
* 导出能力：支持 JSON 与 Excel（.xls XML 格式）两种导出  
  ![](images/20250828105222-0588a3e4-83ba-1.jpeg)
* 自定义正则：内置丰富默认规则，可在「设置」中按分类自定义正则并即时生效  
  ![](images/20250828105223-05c448fe-83ba-1.jpeg)
* Cookie 支持：可一键获取当前站点 Cookie 并保存，便于需要鉴权的请求场景  
  ![](images/20250828105223-060ba5a8-83ba-1.jpeg)
* 去重与过滤：内置增强过滤器（域名/邮箱/手机号/API），减少误报
* 自动与增量：页面加载、DOM 变化与定时策略触发静默扫描；深度扫描过程中分层/分批实时合并与展示
* 可添加自定义正则配置，更好的提取你自己想要的内容  
  ![](images/20250828105224-064f0318-83ba-1.png)
...
1. 右上角开启「开发者模式」
2. 点击「加载已解压的扩展程序」
3. 选择本项目文件夹（包含 manifest.json 的目录）
...
* contentscripts：注入扫描逻辑（content.js 及扫描模块）
...
1. 打开目标页面  
    ![](images/20250828105224-0695122c-83ba-1.jpeg)
...
1. 切换至「深度扫描」页  
    ![](images/20250828105225-06e461b0-83ba-1.jpeg)
...
2. 选择请求方法及要测试的分类（如绝对路径API、相对路径API、JS/CSS/图片/URL/域名/路径）  
    ![](images/20250828105225-072cf7e2-83ba-1.jpeg)
3. 支持选择GET/POST请求方式来进行测试  
    ![](images/20250828105226-07716a06-83ba-1.jpeg)
4. 配置并发与超时，点击「批量请求测试」
5. 在弹窗结果中排序，查看与复制  
    ![](images/20250828105226-07c17d98-83ba-1.jpeg)
6. 支持预览响应

![](images/20250828105227-080f267e-83ba-1.jpeg)

* 导出数据

1. 在「扫描」页点击「导出数据」  
    ![](images/20250828105227-08509924-83ba-1.jpeg)
2. 选择导出为 JSON 或 Excel（.xls）  
    ![](images/20250828105227-0898ed82-83ba-1.jpeg)
3. 文件名格式：域名随机数，例如 example.com123456.xls
