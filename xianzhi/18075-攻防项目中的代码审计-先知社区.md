...
> **来源**: https://xz.aliyun.com/news/18075  
> **文章ID**: 18075
...
![图片.png](images/img_18075_000.png)

搜了下历史漏洞，发现有个权限绕过，试了下可以进后台，但点了一圈功能没打动，没办法只能另辟蹊径：

![图片.png](images/img_18075_001.png)

搜了下互联网资产，有一百多个，还行不算少，后面就是从这些站里面找个其他能拿权限的漏洞，借鉴一下代码看看：

![图片.png](images/img_18075_002.png)

首先查看web.xml，直接搜索upload，还挺多的，挨个看看，首先是/common/Upload：

![图片.png](images/img_18075_003.png)

![图片.png](images/img_18075_004.png)

要先判断这个接口是前台还是后台的，有两个方法，第一个就是直接访问，发现返回302跳转，那就是后台的：

![图片.png](images/img_18075_005.png)

或者找到对应的包，看前面引入的类，如果有session之类的，那多半是后台的：

![图片.png](images/img_18075_006.png)

这里先找前台的漏洞，按照上面的方法可以发现/loan/PictureUpload这个上传接口是没有进行权限校验的：

![图片.png](images/img_18075_007.png)

![图片.png](images/img_18075_008.png)

![图片.png](images/img_18075_009.png)

![图片.png](images/img_18075_010.png)

尝试上传个png，发现上传失败了，并且文件名被识别成了\_.JPG，这样看还和一般的上传包不太一样：

![图片.png](images/img_18075_011.png)

那下面就详细跟下代码看看，大体看下，发现这里没有对上传文件进行处理的代码，先获取了web目录下的tmp路径，然后将其和请求传入了b.a()中：

![图片.png](images/img_18075_012.png)

跟进b.a()：

![图片.png](images/img_18075_013.png)

首先是调用k.a()将tmp的路径赋值给了f.b：

![图片.png](images/img_18075_014.png)
...
![图片.png](images/img_18075_015.png)
...
![图片.png](images/img_18075_024.png)

跟进writeTo()，可发现最后写入的文件是tmp目录+filename：

![图片.png](images/img_18075_025.png)

按照上面分析的流程，我们之前传的文件路径应该是tmp/1.png，但是访问发现tmp目录禁止访问：

![图片.png](images/img_18075_026.png)

而这里的filename参数在整个过程中是没有做过滤的，可以进行目录穿越，于是尝试进行穿越目录，发现还是上传失败，没道理啊，整个上传过程都分析清楚了：

![图片.png](images/img_18075_027.png)

在这里卡了半天，然后返回最开始的PictureUpload，发现服务端返回文件名中包含的\_是从str4获取到的，而str4通过b.c()获取的：

![图片.png](images/img_18075_028.png)

那么跟进b.c()看看，这里有两个for循环，分别处理f和e键值对，而对应的赋值是在最上面的b.a()中进行的：

![图片.png](images/img_18075_029.png)

返回b.a()，这里f其实是post表单传入的参数和对应的值，而e这是上传文件相关的参数，所以上面的流程中会进入第二个循环，获取e的键值对进行处理：

![图片.png](images/img_18075_030.png)
...
![图片.png](images/img_18075_031.png)

a.getFileNames()其实就是name的值，在上传包中也就是file：

![图片.png](images/img_18075_032.png)

![图片.png](images/img_18075_033.png)

![图片.png](images/img_18075_034.png)

a.getFilesystemName()其实就是filename的值，在上面的分析中其实就是1.png：

![图片.png](images/img_18075_035.png)

![图片.png](images/img_18075_036.png)

![图片.png](images/img_18075_037.png)

所以最后e中键值对在上传包中对应(name,filename)，返回b.c()中第二个for循环，调用了b(filename,name)：

![图片.png](images/img_18075_038.png)

跟进b(filename,name)，会先判断filename中是否有jpg、png等图片后缀，然后和this.g实例化为e1，this.g这里是tmp的路径，所以e1其实就是tmp/1.png，而str则是name+b.b(filename)：

![图片.png](images/img_18075_039.png)
...
![图片.png](images/img_18075_040.png)
...
![图片.png](images/img_18075_041.png)

![图片.png](images/img_18075_042.png)
...
![图片.png](images/img_18075_043.png)

![图片.png](images/img_18075_044.png)
...
![图片.png](images/img_18075_045.png)

先搜下互联网资产，还行一百多个，然后就是通过其他漏洞借鉴代码了：

![图片.png](images/img_18075_046.png)
...
![图片.png](images/img_18075_047.png)

那么就再分析一下这个jar文件，接口路由都是由这几个文件控制：

![图片.png](images/img_18075_048.png)

尝试访问下login接口，发现接口存在，那分析的文件就没问题，需要注意的是这里的端口和web端口是不同的，要找这个端口也很简单，直接get请求会返回两个Continue：

![图片.png](images/img_18075_049.png)

![图片.png](images/img_18075_050.png)
...
![图片.png](images/img_18075_051.png)
...
![图片.png](images/img_18075_052.png)

![图片.png](images/img_18075_053.png)

![图片.png](images/img_18075_054.png)

所以这里通过list接口获取系统存在的deviceCode：

![图片.png](images/img_18075_055.png)

然后就是只能上传在E盘的问题，首先要知道系统的web路径，最开始说过该系统后台的sql注入通过报错是可以获取到web路径的，如下，同时比较幸运的是目标系统是在E盘部署的，所以可以通过目录穿越上传文件到web目录：

![图片.png](images/img_18075_056.png)

通过上面的分析构造上传包，上传jsp文件：

![图片.png](images/img_18075_057.png)

![图片.png](images/img_18075_058.png)
...
![图片.png](images/img_18075_059.png)

搜了下历史漏洞，发现有个反序列化和文件上传，不过试了发现目标系统都打不动：

![图片.png](images/img_18075_060.png)

互联网资产有点少，不过没关系，通过上面的历史漏洞可以借鉴代码：

![图片.png](images/img_18075_061.png)

还是先从web.xml中查找上传接口，实际上有很多个，需要挨个看，这里就直接看有漏洞的了：

![图片.png](images/img_18075_062.png)
...
![图片.png](images/img_18075_063.png)

然后默认的上传目录是cms/html：

![图片.png](images/img_18075_064.png)

构造上传包，上传jsp文件：

![图片.png](images/img_18075_065.png)

![图片.png](images/img_18075_066.png)
