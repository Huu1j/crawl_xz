# Google CTF 2023 - v8box-先知社区

> **来源**: https://xz.aliyun.com/news/18035  
> **文章ID**: 18035
...
![](images/20250523192732-eafd98e0-37c8-1.png)
...
后来查阅了新版本，大致是v8 12.x以上的，新增了一些方法，同时编译选项也发生了一些变化，变成了`V8_ENABLE_MEMORY_CORRUPTION_API`，同时多出了一些方法，见下方
...
同时开启了这个编译选项`v8_code_pointer_sandboxing`，实现的[issue id](https://source.chromium.org/chromium/_/chromium/v8/v8.git/+/ee48926106051afb784d8f39c31aab0d2a04823f)。因为我们此时已经又了4GB空间任意读写，所以如果修改了某些函数指针的值，是可以很容易的控制rip，所以这个保护就是来阻止这种行为。对于可执行的函数指针，都会有一个固定的入口（code pointer table），然后根据偏移来索引对应的代码。
...
v8的js解释器定义了一些[opcode](https://source.chromium.org/chromium/chromium/src/+/refs/heads/main:v8/src/interpreter/bytecodes.h;l=45;drc=20911ffd8a0e1636801ddf303c17375fdedf9c83)，这些其中Ldar和Star出现了缺乏边界检查的问题，我们可以利用这两个实现沙箱逃逸
...
![](images/20250523192733-ebf053ec-37c8-1.png)

此时栈和leak的情况

![](images/20250523192735-ed010a6c-37c8-1.png)

由于指针压缩的存在，只能泄漏出低32位的值，同时由于类型是smi，所以会对应的 >>1 ，这里`0x000000000fdb6c0e * 2 = 0x1fb6d81c`&#x20;
...
![](images/20250523192737-eddef7ce-37c8-1.png)
