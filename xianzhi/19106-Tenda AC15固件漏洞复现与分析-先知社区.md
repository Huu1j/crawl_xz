# Tenda AC15固件漏洞复现与分析-先知社区

> **来源**: https://xz.aliyun.com/news/19106  
> **文章ID**: 19106
...
从官网下载固件：[AC15升级软件\_腾达(Tenda)官方网站](https://www.tenda.com.cn/material/show/102680)  
![Pasted image 20250514230701.png](images/img_19106_000.png)
...
![Pasted image 20250521174424.png](images/img_19106_001.png)
...
![Pasted image 20250521174505.png](images/img_19106_002.png)

解包后得到的目录

![Pasted image 20250521174555.png](images/img_19106_003.png)

通过`tree`命令快速定位到我们解包固件后获取到的文件系统`squashfs-root`目录。

![Pasted image 20250521174700.png](images/img_19106_004.png)
...
![Pasted image 20250521202437.png](images/img_19106_005.png)
...
![Pasted image 20250521202437.png](images/img_19106_006.png)
...
![Pasted image 20250521205016.png](images/img_19106_007.png)
...
* 程序文件：/bin/httpd
* 配置文件：/etc\_ro/nginx/conf/nginx.conf

* FTP：

* 程序文件：/bin/httpd
* 配置文件：/etc\_ro/vsftpd.conf

* Samba：

* 程序文件：/usr/sbin/smbd
* 配置文件：/etc\_ro/smb.conf
...
![Pasted image 20250521210447.png](images/img_19106_008.png)
...
![Pasted image 20250521210938.png](images/img_19106_009.png)

这里我们模拟之后发现程序直接卡在这里了，这样我们就需要逆向程序分析为什么会卡在那里。

通过 ida 逆向分析，然后通过`Welcome to ...`字符串定位程序卡住的地方。

![Pasted image 20250521211519.png](images/img_19106_010.png)

定位到代码位置。

![Pasted image 20250521215245.png](images/img_19106_011.png)

在字符串上条指令打个断点，然后动态调试分析程序。

![Pasted image 20250521215613.png](images/img_19106_012.png)

通过 qemu 的`-g`参数开启调试模式模拟程序。

![Pasted image 20250521215403.png](images/img_19106_013.png)

然后通过 ida 连接进行动态调试

![Pasted image 20250521215450.png](images/img_19106_014.png)

![Pasted image 20250521215520.png](images/img_19106_015.png)

启动调试后直接 F9 运行到断点处，然后单步调试分析。

![Pasted image 20250521220230.png](images/img_19106_016.png)
...
![Pasted image 20250521220627.png](images/img_19106_017.png)
...
![Pasted image 20250521220825.png](images/img_19106_018.png)

修改后。

![Pasted image 20250521220825.png](images/img_19106_019.png)

然后保存修改到文件。

点击 Apply patches to input file...

![Pasted image 20250610233110.png](images/img_19106_020.png)

然后点击 OK 即可。

![Pasted image 20250610233138.png](images/img_19106_021.png)

再次进行模拟，这次卡在了一个新的地方。

![Pasted image 20250610233227.png](images/img_19106_022.png)

httpd 监听了 ip 255.255.255.255 的 80 端口。但是这个 ip 地址根本不可以使用，所以我们需要找到修改它的方法。

根据`listen ip`字符串进行定位。

![Pasted image 20250610233619.png](images/img_19106_023.png)

交叉引用发现程序使用了一个`inet_ntoa`函数处理`s.sa_data[2]`这个值，处理结果就是 ip 地址。

![Pasted image 20250610233809.png](images/img_19106_024.png)

这里直接问 GPT `inet_ntoa`函数的作用。

![Pasted image 20250610233910.png](images/img_19106_025.png)

这里我们知道了`inet_ntoa`函数的作用，接下来我们需要知道参数是哪里来的，怎么控制参数才是影响 ip 地址值的关键。

交叉引入发现参数`s.sa_data[2]`的几个操作处：

![Pasted image 20250610234446.png](images/img_19106_026.png)

发现对`s.sa_data[2]`赋值的操作是`inet_addr(a1)`，所以我们需要知道`inet_addr`函数的作用。

再次询问 GPT：

![Pasted image 20250610234912.png](images/img_19106_027.png)

所以这里控制 ip 值变量就是`inet_addr`函数的变量`a1`，`a1`变量是当前函数的第一个参数。

![Pasted image 20250610235107.png](images/img_19106_028.png)

所以我们交叉引入当前函数，观察函数的第一个参数传递的内容。

![Pasted image 20250610235155.png](images/img_19106_029.png)

![Pasted image 20250610235208.png](images/img_19106_030.png)

然后交叉引用`v8`变量，发现影响它的值的是`g_lan_ip`。

![Pasted image 20250610235236.png](images/img_19106_031.png)

![Pasted image 20250610235302.png](images/img_19106_032.png)

`g_lan_ip`是一个未初始化的全局变量，所以我们交叉引入找到对它进行赋值的地方。

![Pasted image 20250610235325.png](images/img_19106_033.png)

找到了`strcpy`函数对`g_lan_ip`存在赋值操作，分析这里的代码。

![Pasted image 20250611000411.png](images/img_19106_034.png)
...
![Pasted image 20250611001134.png](images/img_19106_035.png)

首先分析`libcommon.so`文件，直接在导出表中搜索函数名。

![Pasted image 20250611001311.png](images/img_19106_036.png)

定位到`getLanIfName`函数，发现它只是调用了一个`get_eth_name`函数并且传递了参数 0。

![Pasted image 20250611001249.png](images/img_19106_037.png)

跟进分析发现`get_eth_name`函数也是一个库函数。

![Pasted image 20250611001419.png](images/img_19106_038.png)

搜索`get_eth_name`函数的实现`so`文件。

![Pasted image 20250611001536.png](images/img_19106_039.png)

分析`libChipApi.so`文件，同样通过导出表定位到`get_eth_name`函数。

![Pasted image 20250611001656.png](images/img_19106_040.png)
...
![Pasted image 20250611002606.png](images/img_19106_041.png)

## 漏洞复现

根据大佬的思路，在 httpd 类程序开发的时候往往会直接定义所有需要使用的函数。我们可以通过对常用逻辑的函数如包含`Get`字段的函数进行交叉引用，找到以下定义函数的地方：

![Pasted image 20250617232104.png](images/img_19106_042.png)
...
![Pasted image 20250513234836.png](images/img_19106_043.png)
...
![image-20250930191127308.png](images/img_19106_044.png)
...
![Pasted image 20250513234357.png](images/img_19106_045.png)
...
![image-20250930185247412.png](images/img_19106_046.png)

## 参考

> <https://www.iotsec-zone.com/article/482>
