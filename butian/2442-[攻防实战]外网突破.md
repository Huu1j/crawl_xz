...
> **来源**: https://forum.butian.net/share/2442  
> **文章ID**: 2442
...
![image.png](images/attach-dc15fb381ad5ad415f72b673e089f8a5a86ba421.png)  
找到相关的供应商信息，通过对供应商进行攻击，获取目标单位的数据及权限。!

![image.png](images/attach-06c417df425049e840d6fc969931aea2c57abc86.png)
...
![image.png](images/attach-a8d29cdeabe3761026a599de6f67ed8c5ecf498e.png)

对于heapdump的利用方式这里就不太赘述，有许多文章对其原理和利用都进行了深入的研究，特定情况下还可以直接进行RCE，这里泄露了大量敏感信息，密码信息加入密码本

![image.png](images/attach-66928849da0ca5930c02525ce43efa892d6b634a.png)

登录MinIO，发现大量所属目标单位的敏感信息，也存在其它单位的敏感信息

![image.png](images/attach-c1f383551c52acd3ee64b926f7d93fd9c137aef3.png)

登录Nacos，大量配置文件，密码信息加入密码本!\[\]

![image.png](images/attach-ac9cf0350376d0bdad40573ddba38eb4c5506cbd.png)  
登录OSS，发现大量所属目标单位的敏感信息

![image.png](images/attach-19969a44c514fa8ba5769db3cad69ee31c3e5997.png)
...
![image.png](images/attach-c07cd9ddd04c8648c9c91b6f299567c0b72b41ad.png)
...
![image.png](images/attach-d27d47fad9d778623595d826710881eb124d9568.png)  
成功解密

![image.png](images/attach-cf8e0087265e9df8bccd324c23b00b2c999208a3.png)
...
![image.png](images/attach-34059d3982e40191688ff91a62ff887f20664ed1.png)

对小程序进行解包操作，获取到前端JS代码后中，从中进行提取获得接口

![image.png](images/attach-9d445cec5aa86f847adfa0539ce14de6d811fcbc.png)

直接访问目标接口，前端页面虽然显示初始化失败

![image.png](images/attach-15d32eb56abb7ade12e2457bf0e2831eff541cee.png)  
但流量包中已获取数据，近千万条目标单位敏感信息

![image.png](images/attach-54eca16fac67d0a5ee4681bb3fd48aa33c55adb5.png)

1.3、web程序越权
-----------

通过上述收集到的密码，撞密码撞出一个账号，但是此账号为最低权限，无任何操作权限，点击搜索组织架构，此时无任何返回信息  
![image.png](images/attach-13aebe0c150b3a80913a78bea00857172d90edd8.png)

抓包将parentId和orgLevel去除，再发包，即可越权看到全员组织架构

![image.png](images/attach-eaaa24aec2cd0aaffcba0cb30c7867bba9e0f61d.png)

点击修改密码，然后将上述获取到的roleId添加进去，即可获取全部权限

![image.png](images/attach-766adde7cecebb3b8aa309fd163c508dd0cc9197.png)

获取到大量数据

![image.png](images/attach-aaceb387674ff04511482b780b03369902094232.png)
...
![image.png](images/attach-ff3a1b05088baedcae2323da2f0c6634277c44c6.png)
...
![image.png](images/attach-3ec46704bd4d00efdb7cd3a3a2d9672cacc1e7ee.png)

可以直接拉取下载镜像文件，可以利用脚本批量下载

![image.png](images/attach-f74f3ef03424a1113ec294ac4cbd2b357319fd30.png)
...
![image.png](images/attach-7db617495fcff84526349e5c19fb2c8824e5a18d.png)

通过配置文件连接数据库等

![image.png](images/attach-9663ada321e037f5d9e7399a0c7c5861b57b5c3f.png)
...
![image.png](images/attach-c506302d2a4bd8ad82556dbabd2be3a46eb0eae8.png)  
...
![image.png](images/attach-51cfb8bea7521008030a205378af8c6c852f3660.png)
...
![image.png](images/attach-bc51f0813b8aa1b7f7072167c68ba15a26d10279.png)

通过编排密码爆破进后台，发现大量配置文件，但敏感信息均被加密

![image.png](images/attach-62d51f1c7326e7114b1022865a3669ed6d4f46b7.png)
...
![image.png](images/attach-9faa6a1da39d5898cb65917064c9885c218ded42.png)  
成功连接OSS

![image.png](images/attach-572d2495c72d7b1a2b65dce002b6f11edb94260b.png)

成功连接数据库

![image.png](images/attach-a758622d45da44e07aa10ed7765337f625f449c5.png)

小程序token，接管小程序

![image.png](images/attach-d07c1eb2ba1682ffe1f4a4733a0f5edf186b18de.png)
...
![image.png](images/attach-7965cfa7808da6e0981fbf7b46a710bce3dcf9db.png)
...
![image.png](images/attach-ca978a10493d65b989cf352bbbd125c2f77d8d4b.png)
...
![image.png](images/attach-0069dbaf0c23e4ea2e284cccc370d0508c1f35a0.png)

冰蝎上线并上线CS

![image.png](images/attach-1c88c33f8c747709119140b588136098628a120c.png)
...
![image.png](images/attach-cd3041c5be5bda7370ccbdaa6c958df8b775591b.png)

通过获取到的用户名，使用弱口令进入后台，普通权限

![image.png](images/attach-85e0760d8a43c53ef6903fe8d54e6f8d2ff19d63.png)

再次对公告发布人员猜解密码，成功登录后台，多了系统管理权限，直接添加用户赋予最高权限

![image.png](images/attach-cc108479b419389d1d681b8639c528727d8286a6.png)

新增用户登录，发现定时任务功能，直接使用定时任务执行命令

![image.png](images/attach-ac0b3d70b98d7dd50bfe09bcff238f0ed23089df.png)
...
![image.png](images/attach-f81143a6217ca648a5848eb99aea082c598fb2f5.png)

本着试一试的心态进行了shiro的扫描，默认密钥，直接获取权限

![image.png](images/attach-acd9d83416103fc5d4c208d3b705b28c5a15c09a.png)
