...
> **来源**: https://forum.butian.net/share/2868  
> **文章ID**: 2868
...
![image](images/attach-b138587c81f53735368601a6c9d7a17aa4cb1591.png)​
...
![image](images/attach-478e68a07538fba6162bdaeb3e7658638f5ddb89.png)​
...
![image](images/attach-46a195d0293281c73cafca00dad2e0cf3e4e995d.png)​

再往上跟谁调用了`getObject`​方法，就直接找到了`PoolBackedDataSourceBase#readObject()`​

![image](images/attach-d4d6b4e4653905d057e34da4ce8a598c870272d2.png)​
...
![image](images/attach-5d661bb569be610b1ebd3736aa3c7d8895ee4b0a.png)​
...
![image](images/attach-6a0dfd7e9e6b9dbd76a320ef33f08c8ad4c579af.png)​
...
![image](images/attach-db1b4900ebdd56ed2d889ba54e8257e3fceb876f.png)​
...
![image](images/attach-bcb0c9dce9f4924e5bbc1aea2f0b6e2fe9b444b1.png)​
...
![image](images/attach-dc95433d0e69b65030477e5c89017aee11f6efab.png)​

我们跟进下`indirectForm`​方法，将我们传入的内容强转成`Referenceable`​类 并且调用`getReference`​方法，并将返回的结果作为参数实例化一个`ReferenceSerialized`​对象，然后序列化该对象

![image](images/attach-8453a951600dcad82b4a212457161dba4ecdbb9a.png)​

也就是说我们最终序列化的是一个`ReferenceSerialized`​类的对象，我们来跟进下

![image](images/attach-eefa7e46697681f5312293dc3e859a96cc3a73c0.png)​

再来看看头部，发现其继承的恰好就是`IndirectlySerialized`​

![image](images/attach-11c7f22717661755ccc633751abd26cb05860c40.png)​
...
![image](images/attach-2842ed81291a8819268eea24caabadef26fbbcb3.png)​
...
![image](images/attach-6e77991e3bb2f88f9b6c09295d49e97c4929059b.png)​
...
![image](images/attach-91f80128a31d7fb3345e08b92eace3414a446fcb.png)​

那么可以发现`jndiName`​只要传入String类型即可控制了(之后是有setter方法的)

往上跟进谁调用了`dereference`​方法找到同类下的`inner`​方法

![image](images/attach-2099d0854bb715f760c1b15be37d230ae4bbe5e5.png)​

满足`cachedInner`​为空即可进入下方逻辑

在往上跟

![image](images/attach-1fa595596b735405ca54071971fb58d432d681c2.png)​
...
![image](images/attach-0bb07395a67c82f9880730f0ea55e63dc0946368.png)​
...
![image](images/attach-49bcdb93c77d029862193843430cbdbe5add4db4.png)​

跟进下`getNestedDataSource`​ 发现返回`nestedDataSource`​属性

![image](images/attach-eb4878f95fd574f9376c3bcfa0b193e90c12ff9b.png)​

通过调试可以发现他竟然这里返回的正是我们需要的`JndiRefForwardingDataSource`​类型

![image](images/attach-dfa089e9abee017daa15bbad0961ae97b0e76c09.png)​
...
![image](images/attach-9b0036396ae178620757ed5cfae3335ae9a1f23d.png)​
...
![image](images/attach-92e58619f7330c95fb4726cfef40fe3944161581.png)​
...
![image](images/attach-0098354fc0481c793f77e56c5847469551b4fcf6.png)​

其实很容易就能联想到`Fastjson`​了，因为可以发现`WrapperConnectionPoolDataSource`​也是存在setter方法的

![image](images/attach-10a6157467f55ccf3941457f5b10f3bdf60ab5cb.png)​
...
![image](images/attach-2f2ffda8d636461713702fa0198cb6325af96147.png)​
...
![image](images/attach-200aab839dbe5d8d542ea13b72de5be96fd36e31.png)​

如果都不为空就会把三个参数传入`vcs.fireVetoableChange`​方法中， 跟进下

![image](images/attach-752a345d78412fe7be42f9122a56a1bc3568f5df.png)​

实例化了一个`PropertyChangeEvent`​对象，然后跟进`fireVetoableChange(`​方法，最后在375行这个地方调用了`WrapperConnectionPoolDataSource#vetoableChange`​

![image](images/attach-11fa701314d5e7136ac35a0d218c0f36d473e326.png)​

跟进下，发现跟之前一样会走到`parseUserOverridesAsString`​方法成功进行`hex`​解码并且成功反序列化

![image](images/attach-254cd5ceebc139d889abca1ecb6c3cd8740bbc7e.png)​
...
![image](images/attach-1081b465708b3eade6a97ac5461d3d1499a6a829.png)​
...
![image](images/attach-7ce0018c55b914808eb002881d5037d2dfed683e.png)​

发现匹配`/3.0/authService/**`​ 路径的都会走很多过滤器，在这个控制器里头找到了`parseObject`​方法

![image](images/attach-377be53678e6bbae0c415251d413b6a0fbbb82b3.png)​

‍

![image](images/attach-3c7338218ca648b9e7fc10ea82fcf223ddd75fa6.png)​

那么入口点在这里接下来就是看依赖的事情了

![image](images/attach-b4b4b451de60f952c33880fc35e2b849506d3ff2.png)​
...
​![image](images/attach-99ff1df92a6e54194d03d70fa2dc4aecb8f327d2.png)​​
...
​![image](images/attach-f12e6f93bec03b820f73a88712b5cd84f02665f2.png)​​

大致浏览下其实可以发现他用的类方法都是什么 TiedMapEntry Transformer ConstantTransformer InvokerTransformer

所以可以猜测的没问题就是通过CC6去加载JS引擎加载恶意类，流程图如下

![86a67e0d01f5be54b2a0a970ba212fc](images/attach-1c60f875dc568e2dd36860166a421e2e740f8522.png)​

‍

其恶意类为

![image](images/attach-4c16c98820cf74555923d854c7d3bb756c292e44.png)​
