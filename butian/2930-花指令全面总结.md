...
> **来源**: https://forum.butian.net/share/2930  
> **文章ID**: 2930
...
![image-20230815094310227](images/attach-8637e753a59859246973cfe481e86763d90506fe.png)

当使用IDA分析伪代码时，有花指令会发生

![image-20230815094348064](images/attach-520ea674f31dd932db4d9d384a9ae0b6efe3468e.png)
...
![image-20230815094454421](images/attach-9202183dff617467fcad70bbcfde1a32953b3b22.png)
...
![image-20230815094801422](images/attach-7f5f25821d7c6b3308cdcc773caaab36e73a6324.png)
...
![image-20230815095110085](images/attach-65f75be7309342d07139ab7b4cc5ed0d2756d473.png)

![image-20230815095119159](images/attach-7898cf582af323d4afc010cbb1f7736e5e5fe0f7.png)
...
![image-20230815095303010](images/attach-73326607224002a688dade7ab6c121af511f6097.png)

删除这两条指令的目的是在已经发生了出栈操作并且栈指针ESP与基址指针EBP相等的情况下，不再手动调整栈指针。这是因为栈指针已经回到了调用函数之前的位置，不需要再额外的指令来处理栈平衡。

![image-20230815095343701](images/attach-53906ddec4cd5183127496caa265271454165da6.png)
...
![img](images/attach-a59c324965d96a2c3410b4041e2829c28a5807b6.png)

我对这反编译器对花指令的反编译稍作修改

![img](images/attach-de2475f583417e3cc67c6886f01089d7ef976687.png)

**去掉花指令-> nop(0x90)**

这部分是通过IDA手动去掉花指令，也可以在IDA里用IDApython/IDC写脚本去，或者在OD调试的时候去掉，原理都一样。

![img](images/attach-8a5c21ed298d958b76082f91e354c0523e147f3d.png)

![img](images/attach-ca674f8e6d8b7a5aa3d0be27facc4e896256769a.png)

![img](images/attach-c18497454bcc62648a934754282ba36dfc35914b.png)

![img](images/attach-db864c1b936b1ec81ac75977907e39682842287c.png)

![img](images/attach-216af373ce7959b3b18277133e484b64369b51ee.png)
...
![img](images/attach-7bcf3df844a5a383697a4a136488676d13f04492.png)
...
![img](images/attach-4f08d1497dc06cb904c580f4a7c83c97e51bbb43.png)
...
![image-20230816110539561](images/attach-7405803b7dd1ca7bd829d5725230790f6366bc7e.png)
...
![image-20230816093751805](images/attach-db1d5916c3e22d3da9216df877ce9ddb242ef95d.png)
...
![image-20230816103200617](images/attach-9ce9950eee1962fafd2c91454dcf32293d118791.png)
...
![image-20230816143658327](images/attach-0c2c99f03ded8054653bacb6ebc4b6cd8ff725b4.png)

这里使用了不安全的`scanf`函数, 用户输入的缓冲区只有`0xCh`长, 我们双击`v1`进入栈帧视图

![stack.png](images/attach-60e44a4da58e7f8e5ad181c0ff53b554d8d5e6bd.png)
...
![](images/attach-ee170f775fdd545533bbad6776d4443814fc5ca9.png)

因为 IDA 没能很好的识别数据, 因此我们可以将光标移到数据块的起始位置, 然后按下`C`键 (code) 将这块数据反汇编成代码

![image-20230816144445545](images/attach-e9d4743737359e072d35a88c2e269b803e9fac91.png)
...
![entry.png](images/attach-08ca801c5d49f221919de79767acdcc2c266d8cf.png)

断在`0x413131`处后, 点击菜单栏的`"查看"`, 选择`"RUN跟踪"`, 然后再点击`"调试"`, 选择`"跟踪步入"`, 程序会记录这段花指令执行的过程, 如下图所示:

![trace.png](images/attach-94cd3a73faab3cfb910407c20307c469237ddc6e.png)
...
![jnz.png](images/attach-15eb93de97f5b15ed17b2b930ec98dee08c5ae20.png)

通过修改标志寄存器来满足跳转. 继续跟踪步入 (之后还有`0041362E jnz ctf2017_.00413B03`需要满足). 保证逻辑正确后, 将有效指令取出继续分析就好了

![register.png](images/attach-1ce085ce8d874d40f75a7203c1263302b709cfb1.png)
...
![image-20200616235743442](images/attach-c3bbc29529c659d9dd4eff868e7f3e5d19a76dfc.png)

虽然还是反编译成功了，但是可以看到内容已经完全错误，我们再来看main函数的汇编代码，也就是我们加入的第一个花指令

![image-20200616235852756](images/attach-313a1c3c493faea18a31285b8eab685714650762.png)
...
![image-20200617000152860](images/attach-1cd7ee067f0ab49492d63cec8c29d42406d49a3d.png)

可以发现花指令的混淆作用还是很明显的，那我们继续跟进到花指令的反汇编代码处

![image-20200617000420510](images/attach-c260d9cb54e5728eb5e4d3b283a344a3ad7750aa.png)
...
![image-20230817112338453](images/attach-d2255fec716453040e630c08f92ecf3e3c05f2cf.png)

我们分析一下

因为E8指令(对应汇编call指令)的存在，E8之后的汇编指令被错误的识别成了地址

![image-20230817144205603](images/attach-1d69cc39346b87da260ec86283f7046c7dddd428.png)

按D键转换成数据

![image-20230817144238492](images/attach-fb2225e4e6954b50cccab0e972c56f3eb9d6e250.png)

![image-20230817144300841](images/attach-55d49dc682737350d93ce7b91eb9ab5b332c4314.png)

然后把E8指令nop掉

![image-20230817144351642](images/attach-13362958ac3a332f6287ca7171eb0e9ac4490d6f.png)

把整段花指令nop掉也可以

![image-20230817145240157](images/attach-b0816fd6e9d8e853184b1446b51ebd575b87a556.png)

![image-20230817145248451](images/attach-c322064fdb9b7e8767d1df8cd0d3af4ede559995.png)

再按C键将错误识别的数据转换成指令

![image-20230817144440327](images/attach-51f7e28cbb918c3a5a5288fdd97677f1a347cf4b.png)

最后从函数的结尾到函数的开始重新编译 按P键

`retn` --> `int __cdecl main`

![image-20230817144633355](images/attach-0122b633561439660f62725cac1473025a30c78f.png)

最后Tab或F5

![image-20230817144740904](images/attach-be2737e8316a0d231252d09781c81bd7f2406e0a.png)
...
![image-20230817094651890](images/attach-fbdf3954ef2d29463d26c80cc38111933b6552bb.png)
...
![image-20230817095410879](images/attach-fcd3f7f00d235ba1c64ca361a9de9c7d30c3f926.png)

去花：

只需要把花指令部分nop掉即可

![](images/attach-6884dcae3f0d6022802ec56f7abf8a3b7af0fffb.png)

按D先转换成数据

![image-20230817151540581](images/attach-3f75b45c2d54fa8030dc7e3632f758e1ee701d1e.png)

nop

![image-20230817151533938](images/attach-1b85d3d0277c25bb9cf778bb37025bb75c8b0965.png)

加上未识别的数据重新反汇编

![image-20230817151618118](images/attach-1a6cd6d62334eb055e9083dbe9ad0f6d9835a175.png)

![image-20230817151632843](images/attach-bd6aeecb74c6ce3e9590475abbb1fda974a315b2.png)

重新反编译

![image-20230817151653048](images/attach-112c132a7a818cfd6fde0b2a40f3d38b68055432.png)

##### IDC去花

先获取`jz short loc_45E421`的字节码如下图，我们只要取`74 03`即可，因为`0x74`代表jz助记符，而`add reg16,reg16`的字节码刚好为 `0x03`，对应了`add eax,eax`

![image-20230817151924552](images/attach-ee0d9fd5c2ce9e360b122be2e26d1923bc444540.png)

![image-20230817151952590](images/attach-b48dc7ee6ac35facebb798e011507de876ab9b2f.png)
...
![image-20230817152102477](images/attach-20098495a7e6fa8ac9be09c4fa319dafe1a6c4f7.png)

![image-20230817152124837](images/attach-4d9c446a8f55d412b0afecb6c618cfd94541afe8.png)

![image-20230817152348358](images/attach-d539e0c1c9bd3b03e7c601d5bd1c41806468a543.png)

![image-20230817152410567](images/attach-4c0d45fe8de6bc742eb4441ef0bd76bc4c7a2de0.png)
...
![image-20230817154813486](images/attach-da1c72a4ec697c89fb2cacc4152e983c814d4213.png)
...
![image-20230817162527744](images/attach-6d85cd3d23208aaa6fc61bcd670962b7a4606e66.png)

按P重新申明一下函数

![image-20230817162606813](images/attach-be1e0a3ea44de358b9ca153af5923a10a2c62d80.png)
...
![在这里插入图片描述](images/attach-62404ddfab61029747f9178b5435adddc915d8af.png)

修改之后变成`mov eax, dword ptr ss : [esp - 8];`（在OD中没有反汇编成功）的地址

![在这里插入图片描述](images/attach-d5cd32c4f973da4728839bb80ca5c35f2eb2a588.png)

一共三次出栈操作，第三次是将修改后的指令地址出栈到eax，下面有个jmp无条件跳转语句，程序自动跳到`mov eax, dword ptr ss : [esp - 8];`，绕过垃圾指令

![在这里插入图片描述](images/attach-eeea76ef7da3472c7fe7c38868a9dda9ff902978.png)

#### （二）去花

**1.反汇编器手动改字节**

![在这里插入图片描述](images/attach-be4357aff2d9006e4f192aaa891900d6942fe4a1.png)

![在这里插入图片描述](images/attach-f000c47f9a861ab5a4ddbe1cc18247d5f690fa49.png)

然后再把位于`jmp eax`下面的`call`语句`nop`掉

![在这里插入图片描述](images/attach-2552637671b30232b1ad0262e09689eec17ea16d.png)

![在这里插入图片描述](images/attach-39b354e1f9f3dbc5b5c9aca00d7e772b46479dd8.png)

效果如下

![在这里插入图片描述](images/attach-0c5fd7003d6dc004fce006233a0c44787b3dfedd.png)

但光这样会出现问题就是因为`jmp eax`的存在，`反汇编引擎`不知道跳转的位置，所以部分代码会丢失，被分隔开，产生如下情况；

![在这里插入图片描述](images/attach-02283ebe8c7d0baabbc278391c202d1c5526ecc9.png)

所以还需要把`jmp eax`用`nop`填充掉，这样就可以F5了

![在这里插入图片描述](images/attach-85cddc9ed9f99c6ea3773bb0878c97c05a17cc2a.png)
...
![在这里插入图片描述](images/attach-9621b268d1c455a89c98ec4a398ac496adc57720.png)
...
![image-20230817164108385](images/attach-73c31d6b788b8545549243f3f6206d9c574cfaf6.png)

![image-20230817164124171](images/attach-4c47631d09d3f9104be8cb75622bd5f770f40907.png)

![image-20230817164132287](images/attach-3624db69e29f351811337c4afc3d1229fd5f2978.png)

![image-20230817164331190](images/attach-60064789f3d6f674dfb861e2cf6b270a0cee5baf.png)

![image-20230817164344960](images/attach-75600f099f3ecfc6cec428badcb94804532eb50e.png)
