...
> **来源**: https://forum.butian.net/share/2569  
> **文章ID**: 2569
...
![image.png](images/attach-637e19268c54a8dc2655e27c564667ecff790774.png)
...
![image.png](images/attach-3ffd9a41ed10b6dbc75399b7a63db15d96297511.png)

 若想在找到实际资源之前进行拦截，可以考虑在对应的Filter中使用@PreMatching注解，可以在启动请求匹配之前执行的请求过滤器，即可以修改请求的资源，影响匹配的方法。同样是上面的例子：

![image.png](images/attach-27b5194f90a10bb4b21622a6a6a156b9653df23d.png)

 可以看到当启用PreMatching注解后，访问一个不存在的资源/test时，会经过AuthenticationFilter的处理并因为鉴权认证不通过返回401 status：

![image.png](images/attach-f83d598c021c10761f9090f1a0096e26394763f2.png)
...
![image.png](images/attach-b1e13cd7d50f8ed8050143803ba78c499183da7a.png)
...
![image.png](images/attach-19245ab312fe9846713abc4406a820549fb294a2.png)

 可以看到当以非admin用户身份进行请求资源时，AdminAuthFilter生效，返回403 status：

![image.png](images/attach-2c10b321671eb2d9b4d7299c1ad74f6939ca1947.png)
...
![image.png](images/attach-ab10d8663b9b5497de7b811682e9060082851e6f.png)

 还是刚刚的例子，假设AdminController下的所有注册资源都需要经过AdminAuthFilter处理：

![image.png](images/attach-97819bcff40254b949860b77254601a54b42b7dd.png)

 当该接口注册生效时，可以看到当以非admin用户身份进行请求资源时，AdminAuthFilter生效，返回403 status：

![image.png](images/attach-60dee2a0e9384abeaa24f45f327b8508781716b5.png)
...
![image.png](images/attach-cada3924bac255f1a40265f4c97cdfcc33536ef6.png)

 可以看到非登陆情况下禁止访问：

![image.png](images/attach-7746b196873297eeee1ba680cf75af7b137545eb.png)
...
![image.png](images/attach-330aff497fd44345f51ec771881ac05e4f0fb002.png)

 可以看到当以非ADMIN身份尝试访问对应接口时，提示没有访问权限：

![image.png](images/attach-65571615cb2bdc88d3065109c5f4254d82089159.png)
