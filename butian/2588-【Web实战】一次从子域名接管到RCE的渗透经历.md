...
> **来源**: https://forum.butian.net/share/2588  
> **文章ID**: 2588
...
![image.png](images/attach-12a7d0e7393aad0f069edf365402d547b25fcd7e.png)  
...
![image.png](images/attach-9f27c8f8284ef12490f661f7d91d12a045794aa1.png)
...
![image.png](images/attach-5b9dfd45b80bfeac4f2f3acaa28767400ce9cbff.png)  
如果username存在，就会返回code200的json  
![image.png](images/attach-29eab895ed8fec687231638db0a45888874f6ec4.png)  
...
![image.png](images/attach-f4ab7fdecc978e3e905f034239f2e04379c2f78b.png)  
于是我查询了cdn1.site.com的解析记录，发现指向了XXX.github.io。我去搜索这个“XXX”这个用户名，但是在GitHub上搜不到，直接访问XXX.github.io也不行，我推测可能是这个账号已经注销掉了。

![image.png](images/attach-2edca3e84727c1388f7522419a794b39c892c785.png)  
前往GitHub注册页面，输入“XXX”,发现用户名可用，完成注册操作，新建一个js仓库

![image.png](images/attach-c2509ff53b43e2778d58753ca5d5ccbde1d668cd.png)  
然后打开GitHub Pages服务，将仓库部署到服务器上，显示“Your site is live at \[<https://XXX.github.io/js/>\]() ” ，在下面添加cdn1.site.com。  
![image.png](images/attach-9805cf96e1915dcede7fee0c1ff3ea01edc57380.png)  
我们新建一个名为jquery-2.2.4.min.js的文件，并且写入payload

![image.png](images/attach-982cac4d2fe9b06539de2a29e687a23b959a568c.png)  
通过cdn1.site.com访问这个js文件，发现正常显示  
![image.png](images/attach-1abc10bf239d64bf652b0a61c19333903db43883.png)  
访问OA登陆系统首页，成功写入XSS payload，域名接管成功  
![image.png](images/attach-d5fd8c61150c54860248c758b60ad5b8f83566ef.png)
...
![image.png](images/attach-c1029a33b8ac4e894046a04338727c6144b258c0.png)  
...
![image.png](images/attach-28310176e1f9eeaf53b3a55f25b361d67663a233.png)

![image.png](images/attach-2b51a46c5fa354aaba8c488dcdb1bfc468abae66.png)  
...
![image.png](images/attach-9cecc1685562b3bf5e1540817c53392d81d24939.png)  
成功有鱼上钩，我们拿Cookie试一下  
...
![image.png](images/attach-c5c48d02d9ce0c734947d158c7a634a5b1bb6477.png)  
我们刷新页面，发现自动重定向到后台了，里面有很多会员的身份信息，均为敏感信息。我们做到这里不得不吐槽一句这个单位的数据安全防护太不尽人意了，先后两个系统数据都被泄露。  
![image.png](images/attach-691eeb2c139029caff3dbb5eb23d4ef4c3724b4e.png)  
...
![image.png](images/attach-b86b5f4be1134d02967edc9e60d09fe377181732.png)  
我们把文件解压后，发现真的是一个很简单的数据管理系统，可以说是裸奔在互联网了。我们直接来到untils目录下面，这个目录一般就是放一些配置文件之类的  
![image.png](images/attach-f4354928675f138c35e1e5bf149dfa5a9205f879.png)  
先打开connect\_db.php

![image.png](images/attach-36bd0408de4c618e2847c44739e1d0622ecf4179.png)  
简单、直接、明了地告诉了我们数据库密码，够爽快！但是数据库只允许127.0.0.1本地连接，我试了发现无法直接连接数据库，放弃sql提权的思路，看看有没有其他更简单的方式

![image.png](images/attach-b7aa8eb1001476a2403d6b455186f087bdd47bf7.png)  
在admin目录下有个文件上传功能，是用来上传用户头像，但是好像这个功能没有继续开发导致荒废，不过代码没有删除（或者这个功能正在开发中，只开发到了一半，没有写前端）

![image.png](images/attach-77c399608542ddb00290c51ce67087c963f362f1.png)  
...
![image.png](images/attach-0ad628df0e13298e66e3b7ff214154aea2107f21.png)  
我们访问含有index.js的页面并且带上`?us=2`，就可以生成以下页面  
![image.png](images/attach-d9131bb58f0b964b4158319bc0ebcfa90462905e.png)  
我们上传一个图片先抓包  
![image.png](images/attach-c55895100646a312ad9ef32ffa952a7daa3bf9f2.png)  
右键->复制为powershell

![image.png](images/attach-ef826837bfaed138df36dce8ce62d3b18e4a37bc.png)  
...
![image.png](images/attach-e19ee41aaa7c404c5e81210a5247cb7902b658f4.png)  
只要这里是图片格式就行，我们可以把文件名修改1.php，写入一句话木马，完成上传

![image.png](images/attach-82f5809c59225bd3fd603d15ad3ca8fec587408e.png)

![image.png](images/attach-a2e717d52e4a47852d3820b9d905e4605d852d90.png)  
这样我们就可以完成上传，找到文件储存的地址admin/touxiang/1.php

![image.png](images/attach-d3ebb7b44c9ee79a126b4a417d8d0941ee6fd066.png)

![image.png](images/attach-b9d4086b9e831afdf5a7535dc90b89c79e62b1ae.png)  
也是成功写入一句话木马

![image.png](images/attach-24fc2a0ca82ef50aa60db98628cec270069fdaad.png)  
成功连接上sql数据库
...
![image.png](images/attach-ef1a2b656e46b6fa8e637254bb7f1853f44d9c9e.png)

![image.png](images/attach-39051009bf3c91f0815a6d21f518447e16c5bce0.png)  
creater.py
...
![image.png](images/attach-b4368ffcab0657764cfb0808f8b58e748f86515c.png)
