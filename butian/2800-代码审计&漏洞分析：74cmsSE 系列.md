...
> **来源**: https://forum.butian.net/share/2800  
> **文章ID**: 2800
...
![pFsFXPP.png](images/attach-6c69916024593c07e875d2179c27f89ec1d95305.png)

CNVD  
![pFsdUaj.md.png](images/attach-3e7b051bf41549d363a5e73c4385c6e5d9f900b9.png)
...
![pFsFL5t.png](images/attach-671490d5b0ea6bf42f9a2b5cb99f23b5b0e43faf.png)

代码分析：  
定位到核心函数 index() 处，$url 和 $ourput\_filename 参数均由封装的 get 方法获取，跟进到 request()->get() 里。  
![pFsFqUI.png](images/attach-1aff673d824f3e166496de30fce26d2006fd3a6b.png)

从 `$_GET` 变量中获取到请求的数据并存储在自身的属性 $get 中，再跟进到 input() 方法中。  
![pFskprQ.png](images/attach-75ea3ca31ec651876428040396259ef4df2d32a9.png)

input() 主要对传入的格式进行 trim() 操作，没有其他过滤。  
![pFsAxBQ.png](images/attach-5ea4e8ca9727a2b1c814b2fe7d2498cc473df328.png)

因此这里 fopen() 中的 $url 可控且没有安全过滤，简单构造即可读取任意文件。  
![pFsFv28.png](images/attach-3e5bab81c83892ac63a993e496a93417d92d77e4.png)
...
![pFsFxxS.png](images/attach-282b65ddf169ac182b2e2b40205de2257bec31ff.png)

#### 代码分析

定位到关键函数 resumelist() ，接收四个参数，其中 $keyword 接收字符串格式，这里输入 sec 作为测试字符进行跟踪。  
![pFskSKg.png](images/attach-ab7208256561ef31abbc85a699473f07265bc36d.png)
...
![pFsk9bj.png](images/attach-781a989cc5a8cc972befacbc3f0e21dac74d0c7d.png)
...
![pFseTr8.png](images/attach-ab2e0a19e78c48ed5a4760995629adc70761e86c.png)

#### 检查过滤代码

跟进查看，这里只对输入做了 trim 操作。  
![pFse7qS.png](images/attach-63779f88f1ac89f43771d71dd0458f0534bc3196.png)  
...
![pFseL5j.png](images/attach-8afb8fce8b6e7ea0988bad197a49666d8ba55aeb.png)

#### 后续的修复代码

查看最新的源码（版本3.28.0），过滤使用了 addslashes 操作。  
![pFseoKf.png](images/attach-d0e7601ad5ec8034353d3dd8b66dfbdce35c4e64.png)
...
![pFseqaQ.png](images/attach-bd5287752890158cc2d15f1cfd664601ea4b3384.png)
...
![pFsebVg.png](images/attach-a45b6dca84a26f86d2379e68bb9971348a0492ee.png)
...
![pFseXPs.png](images/attach-6282046c5c51050dd7820072614dc88467086e1e.png)
...
![pFsejGn.png](images/attach-6e81cec53f3c855cc2483af5797d9425313a1f6c.png)
...
经过测试，发现输入的内容会被逗号 `,` 隔开  
![pFsnBnK.png](images/attach-3c01ef91d0f7981abb30ff639ce9a9882c6c2ca1.png)

查看该函数的定义，若要进行闭合，需要逗号构造，但是上述测试发现使用不了逗号。（暂时没有其他思路）  
![pFsnwX6.png](images/attach-b150dd7781ca9b5d79054991c633437a30f34d02.png)

$sort 参数只有在特定字段时会出现在语句中，暂未发现利用思路。  
![pFsn29A.png](images/attach-5a49b3a5f2ada101eca8bd7ffac333ef6bdd7897.png)

map() 函数中的注入漏洞也是一样，出现在 $keyword 中  
![pFsnchd.png](images/attach-dd5c25258909f6aacb19e1a455d93d2239c27988.png)

### v3.5.1 SQL 注入｜Resume.php

剩下几个漏洞触发点都是相同的 $keyword ，故不再做分析。  
![pFsnR1I.png](images/attach-55c020182877901a08bb313e2998aed1d67acc76.png)
...
![pFsnr7D.png](images/attach-23af6330d7457e42265bbcdbe7c5f97f7d5a7214.png)

以 ceshi1 用户登录后台，可以直接操作修改 admin 的密码  
![pFsnyAe.png](images/attach-a56cb3da6354a692f3da9cb1b902da32f58c3524.png)

代码分析：  
查看触发的函数 edit() ，路径位于 /application/apiadmin/controller/Admin.php  
跟踪发现，其中并没有完善的鉴权机制，只要能访问到这个页面就可以进行修改。  
![pFsnD0O.png](images/attach-e0f3805e9bf79d6643f4f1df5102931d2bb8f2e5.png)
...
![pFsdlGt.png](images/attach-1679f2935e0568c70bd4dfffd6a8e2295800ceb1.png)

代码分析：  
后端对输入点只做了 trim 过滤操作，前端也没有有效的过滤  
![pFsd1RP.png](images/attach-967910d2021c70d082c648cfb0a8451e976a0abf.png)

前端也没有发现相关的过滤，搜索到了 dompurify 关键字，但是没有使用。  
![pFsdGM8.png](images/attach-3e0e23c49e68ea92cebfae105f966c08cf874d29.png)
...
![pFsd3xf.png](images/attach-a5aa097d1838dd9491651e2ad0bbae33160014c9.png)
