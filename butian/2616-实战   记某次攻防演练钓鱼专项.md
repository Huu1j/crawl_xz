...
> **来源**: https://forum.butian.net/share/2616  
> **文章ID**: 2616
...
出现如下提示说明25端口正常  
![image.png](images/attach-b338189aead1f2d892b2b3117ccd247e78f22201.png)  
...
出现如下提示，说明安装成功：  
![image.png](images/attach-4949b188e94c9bebd92b97ad16af3fd71f72d6ce.png)  
然后需要购买一个具有迷惑性的域名，用来降低企业员工的警惕心，可以到godaddy上购买一个，也不贵，购买成功以后，需要配置如下域名解析：  
![image.png](images/attach-a3226c64bf769eb1bf01770ceef0a93f23c52eef.png)  
...
![image.png](images/attach-4ade447099cc4376da195bff997de2e576cb89e5.png)  
在上文说到有一个TXT的解析记录需要到Web控制台来生成，用来反垃圾邮件如下：  
![image.png](images/attach-8c304b838ffe2e30e0b33f34511278c74c1ef9c9.png)  
然后登陆邮箱后台来测试收发邮件，先给qq邮箱发一份，发送成功如下：  
![image.png](images/attach-ccffa60ae566d81652ecf6e8711fcaab0f075b01.png)  
qq邮箱成功收到邮件如下：  
![image.png](images/attach-79b661bc1a5bc9de04fc1b29a90f0f181ba499d5.png)  
...
![image.png](images/attach-21bbbbe6187c8d2486dc150cd504ecdc2a02e7ed.png)  
然后联动之前搭建的邮件服务器如下：  
![image.png](images/attach-edc93155e6b864bf41d4feec81029dec6e5059e8.png)  
点击测试发信，邮箱成功收到测试邮件如下：  
![image.png](images/attach-7849d2475e5858fc41c149484f2f3f943fbe6bfe.png)

### 目标邮箱获取

通过对目标企业外网信息收集，发现一处springboot信息泄露，下载到heapdump，获取到大量该企业邮箱如下：  
![image.png](images/attach-c0896c94b846b9bf3ccd2956d8c00f8d406bc530.png)

### 邮箱模板/网站模板制作

通过调研，发现该企业使用的为某讯企业邮，通过此信息，构造异地登陆邮件模板如下：  
![image.png](images/attach-84634e27ecdf3a00d630f6b338bd0779aedc3f2a.png)  
点击登陆后跳转到伪造的钓鱼页面，然后获取员工的敏感信息。这里涉及到钓鱼页面的制作，Gophish提供了一键导入功能，但是有些页面直接导入会出现乱码问题如下：  
![image.png](images/attach-163f64e105ee89bcd2c6d82217b6590d7dba96bf.png)

所以，这里需要使用到一些扒网站工具，或者一些浏览器插件来完成，这里选择使用浏览器插件`Save Page WE`，会将网页直接保存成HTML，然后本地打开，需要将form表单的action清空如下：  
![image.png](images/attach-57c3ea6fe2abaeaecfee68874aac89d3653e4798.png)  
然后将HTML文件复制到Gophish再次查看页面，发现已经正常，非常逼真如下：  
![image.png](images/attach-39e31d720ee8090fa162d0457dcba9e287ac70d6.png)  
直接发信测试，发现已经有员工上钩，输入了相关数据如下：  
![image.png](images/attach-c1868025eba0ddcf862929453d68a28c2f8ed35e.png)  
![image.png](images/attach-c7811ad6082229ae33e663b5352c8c1445783f49.png)
...
![image.png](images/attach-11007427199c778d314765f0fcb4cf4d4f3ab6af.png)  
具体成品如下图：  
![image.png](images/attach-3f76688095b9822d40b2d04fa0b89ae0d8b857c0.png)
...
编译好客户端运行如下，和CS操作很类似。  
![image.png](images/attach-bc37fc83b092e69dbe611bd4f36acfe089a2e58b.png)  
做好免杀马之后，还需要考虑一个问题，**那就是如何绕过邮件网关**，前面我们已经知道客户使用的是某迅企业邮，**而某迅企业邮，只需要双层压缩，最外层设置密码即可绕过**。现在先自测发现木马免杀且可以正常上线如下：  
![image.png](images/attach-dc51ad730cba82cb07bce49b2ff454d47866255b.png)  
开始使用Gophish进行批量发送，**Gophish支持设置时间段发送，这样可以降低发信频率，从而降低进入垃圾箱概率**。发信结果如下：  
![image.png](images/attach-73442535b2832d5da60a4667873767fa5e605fb6.png)  
大部分员工会点击查看邮件，这次效果不错，最终有6人双击附件，控制其终端，对其中一台执行命令如下：  
![image.png](images/attach-6a63705f9d0377b79f5f32d50f3a43afd5fab73c.png)

### 最终成果

本次钓鱼专项，共发送269封钓鱼邮件，**其中共计65次钓鱼邮件被打开，2次员工信息输入，6次鱼附件木马被运行**。详细图如下：  
![image.png](images/attach-9d7b69f7d8c6eb40449b6fca4ea7097f82354920.png)
