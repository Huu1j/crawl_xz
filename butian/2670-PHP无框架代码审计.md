...
> **来源**: https://forum.butian.net/share/2670  
> **文章ID**: 2670
...
![1.png](images/attach-acf10ebd731bc4a19d2f99f2158faef4e6bf07a1.png)

0x01 目录结构
---------

开始审计前，先看一下目录结构，判断是否使用框架开发，常见的框架如Thinkphp、Laravel、Yii等都有比较明显的特征  
![2.png](images/attach-24174165322810a31e27c75fc35f2096bb981e5b.png)
...
![3.png](images/attach-acda7acd3fd84acca16f640c50288042c0e50d10.png)

然后通过条件判断来确定 $mod 的值，然后跟进 $mod 的值定义`SYSTEM_ACT`常量  
![4.png](images/attach-209e969681bfc4ac3df1e090bf80209f4ca42cc1.png)

接着根据是否传入参数do和act来确定参数的值  
![5.png](images/attach-3305f9c3318e5438cc89889bd2742ddf7c7888d9.png)
...
![6.png](images/attach-4773e2b6e7f257b1ff217e722c08238b45d277d7.png)

随后发现该文件中定义了一个irequestsplite函数  
![7.png](images/attach-62ccba8f94371f6750498816ff91a81e5d66a72e.png)

irequestsplite()函数主要是用htmlspecialchars()将预定义字符（&、<、>、"、'）转换为HTML实体，防止XSS。92行对$\_GP调用irequestsplite()处理，即对GET和POST传入的数据都进行处理  
![8.png](images/attach-ca3cdf9b00c8bd3bd3a226c02defb3a2523ea59d.png)
...
![9.png](images/attach-f976038b244e621b600c4cb0d8e0f995bbf1a1bb.png)

在登录页面，可以看到四个参数mod、act、do、beid，这里主要关注前三个，将这三个变量接收的参数在网站目录的文件中寻找  
![10.png](images/attach-dcfa1cd1fa52a150972c51336fcbea91e87b1b57.png)

可以看到接收的值和标记的文件目录文件名一样，index.php调用了page，查看一下  
![11.png](images/attach-c5636e4a891c750c754c11ac9e0c8b00300e995f.png)

会调用/template/mobile/目录下的index.php文件  
![12.png](images/attach-3e032ca9da4724168cc526366be1b3f5a70a7fb9.png)

确认是正确对应，act代表目录名，mod代表目录名，do代表文件名

登录后台页面，查看url，site、manager、store三个参数  
![13.png](images/attach-a4dad27261d60567e075c7bac6cf7d0adc72185a.png)

继续看网站目录的文件，发现web目录不符合  
![14.png](images/attach-25ffc5a31b319233f38ec300f3c53fed3ce7c960.png)

尝试修改mod值为web，发现可正常访问  
![15.png](images/attach-79b9357daa83ac6c8681d58439b1db0f01af16be.png)
...
浏览网页，发现搜索功能  
![16.png](images/attach-3a8c827a2f5b1abc96500550fab0bfc3aa193035.png)

根据url定位到文件\\system\\manager\\class\\web\\store.php，抓包发现接收参数为sname，搜索sname  
![17.png](images/attach-fd26044bf80832fc106a93b85bb4ce709b60705e.png)
...
![18.png](images/attach-a2c597fe464a93184f7d9db6022836142f02ee22.png)

在excel.php中，检查文件后缀是否为xlsx，无法上传，看第二处common.inc.php文件  
![19.png](images/attach-51585451e3b4eb6af8acfed57c8f8d0a372082d0.png)

在`file_move`自定义函数中使用了`move_uploaded_file`函数，移动上传的文件，跟进`file_move`  
![20.png](images/attach-4565b747f98769757a37e67db2bd88efe653727b.png)

在`file_save`函数中调用，继续跟进`file_save`，找到4处调用，逐个审计，发现只有一处对文件后缀没有限制  
![21.png](images/attach-75e972dd4e9f9a73d36c45e49b554eab16bf8a26.png)
...
![22.png](images/attach-590afaeb2c49bf5a2a82b1b28467dd253c45886c.png)

可以发现上传的数据通过url参数传入，传参方式为$\_GPC，等同与$\_GP  
![23.png](images/attach-5d1785fa0855ed0d36f8fbcff1a087e6cea2ddf2.png)
...
![24.png](images/attach-2f0a5850346a6e6382ae3e95bfd93f183a1effea.png)

访问该路径，成功写入

![25.png](images/attach-0d67ca395889ee6f4b40025c4665b66f4843f86d.png)
...
![26.png](images/attach-6bf7cc3ff5843cc049cce497e5cc64cffc322935.png)

寻找`file_delete`调用的地方，看参数可控处审计  
![27.png](images/attach-3135c891be15d52a2fbd26a79c31576669835701.png)
...
![28.png](images/attach-3f6b8da6b9d080f9ef0deb1d0257ea8487dbf917.png)

`/index.php?mod=mobile&do=util&act=uploader&m=eshop&op=remove&file=../aaa.txt`

![29.png](images/attach-b64ebd1aaf10228015a7be9eaa70a26712dc8406.png)

![30.png](images/attach-9f16bd767ac42fab3f10c60c877992c0dc9a253a.png)
...
![31.png](images/attach-891c1b1e8bbd270bb0a07fed907c9a11b4f554a6.png)
...
![32.png](images/attach-e8709ce2dd567da8a505dabeef003af6839009d3.png)
...
![33.png](images/attach-defabb8f056d07697b7c8fc5e647c90c7a28b992.png)
...
![34.png](images/attach-d9b9cdef5bc84e2da61a98be9d61eb749adff2cf.png)

当`$settings['image_compress_openscale']`非空的时候，就会调用system()，参数拼接的，其中`$file_full_path`是通过函数传入的第四个参数

搜索`image_compress_openscale`的值  
![35.png](images/attach-aa0defe151c54d206bdcea3e37799181dfcccb57.png)

可以看到是通过$\_GP传入进行设置，发数据包设置为非空即可

接下来寻找`file_save`函数调用的地方，主要关注第四个参数是否可控即可，在`/system/weixin/class/web/setting.php`中找到一处调用  
![36.png](images/attach-c6c5e9e9d1db85a5091fa039e5aceeed977ea370.png)

可以看到第四个参数是根目录路径加上`$fie['name']`

![37.png](images/attach-610e1aaf2944e4d3926b3881cafe3a7141662985.png)
...
![38.png](images/attach-70731209820a130840d3fcec208b9b3ce81438d6.png)

上传文件抓包，修改文件名（因为前面会拼接路径，所以需要用&进行分割）  
`filename="&whoami&.txt"`

![39.png](images/attach-86b0a7371633318e5a972dadd601e8129864b49c.png)
