...
> **来源**: https://forum.butian.net/share/2905  
> **文章ID**: 2905
...
![1](images/attach-0e5e370a893a525e8194fc2d758e7b086adef6f5.png)

后边经过作者的同意，然后把本次代码审计的思路放出来和大家分享一下

![2](images/attach-9bd2afdfc8d204a25c264497b0c59b234515e6ad.png)
...
![image-20240403021320315](images/attach-3ec16f686e19c50c3aed3322c7f04e03fe7ea1e1.png)

我对mybatis-plus的理解就是封装了一些sql以减少代码量，其对绝大部分场景进行了预编译处理。但是类似动态表名、orderby这种需要拼接的场景配置不当还是会存在漏洞。

在翻工具类时发现该项目有对sql注入进行过滤

![image-20240403025212716](images/attach-36e3b6fba42eb82a140b77741523187049cd8331.png)

那就跟进去看看在哪里做了过滤，只在`com.utils.Query#Query`中有4处调用

![image-20240411115923581](images/attach-0903ffbf5f94ee48b9e32c3c59da6937f35783e9.png)
...
![image-20240411120600013](images/attach-bc247895058bcbcfea74900714b96db15c3ee30c.png)

但其对sql注入的防御逻辑都相同，都是对sidx和order参数进行处理后，然后封装成Page对象

![image-20240411121107373](images/attach-04ddc9a0714ff7579d6865a241f575a45c43e199.png)

![image-20240411121146368](images/attach-09faa249a96a894a46edd9b93c0043fa5d5c01d4.png)
...
![image-20240411121930477](images/attach-1754874526eae6ebf856fadfcaea5d725808c48a.png)
...
![image-20240403032355989](images/attach-0cb0b94aebcf94e084a0fa7226a79ef606392740.png)
...
![image-20240403032630418](images/attach-60e3aaab1e8e2298a8bcb71efb8291dbeadd5378.png)

继续向下跟

![image-20240403033241569](images/attach-feab81eded99648a88a1d9441f9636f2b1f787cd.png)

![image-20240403033302136](images/attach-ae243f349b68d1f1b686b03c70369048362f709b.png)

可以看到最终sql的实现使用的是开头说的自定义的`wrapper`来构建查询条件。

![image-20240403033810081](images/attach-18859e800509145cc0b01a4f7df1075cb4a12989.png)

跟进去看看->`com.utils.MPUtil#sort`

发现在这块对`sort`参数直接使用了拼接而且没有进行过滤

![image-20240403033917326](images/attach-d65afa11acac07217a766aa6d8f7d3b3f3e1765f.png)
...
![image-20240403034431648](images/attach-c2ace7485898ef14dac13bb3806ddf0cd7f58b96.png)
...
![image-20240403040500351](images/attach-e4fcc43894f16f19287dc08ee39c4234454a99c8.png)

可以发现多出使用拼接，那直接选择一个分析一下

![image-20240404211942229](images/attach-4e172dad4506fc7ffb8836fed3021416ccf6553b.png)

一路向上跟进  
`com.service.impl.CommonServiceImpl#remindCount`

![image-20240404212053916](images/attach-74fed281e8c9932ea338b26081d04dbd1d4ff879.png)

最终跟到`com.controller.CommonController#remindCount`，发现全程也无过滤

![image-20240404212127974](images/attach-18222ea664e37973824a09b9edea1a605d8fa6d7.png)
...
![image-20240404211110262](images/attach-7765ca1537bf6085a47fe41097f3ac5ecf3a5846.png)
...
![image-20240404232632119](images/attach-7cf2f6a7bec9da3a954dfcec705011671c3239f2.png)
...
![image-20240404233310202](images/attach-6079c0b5a461019c22219710ada4a906535b3b07.png)

0x04 任意文件上传
-----------

![image-20240405012738083](images/attach-607340d7c49aa46f4c1efe93691b0366d11fe005.png)

### 分析

在`com.controller.FileController#upload`中实现的文件上传功能，通过最后一个点来获取上传文件后缀，在结合时间生成心的文件名，但未对文件进行过滤。导致任意文件上传

![image-20240405012907744](images/attach-dd4cefbb224ea3ead9f9ae9d8ed150a3ebcf3162.png)

0x05 任意文件下载
-----------

![image-20240405012147812](images/attach-586aec266639dc1acbf84ae08b269adde3010506.png)
...
![image-20240405012326337](images/attach-8055e0d02d53e4e58c38cb0504957ff7b3c1df3a.png)
