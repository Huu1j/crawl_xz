...
> **来源**: https://forum.butian.net/share/2734  
> **文章ID**: 2734
...
![image-20231222210239843](images/attach-87de2a74fa167e99251e4ed3900e6b9d65ea66f0.png)
...
![image-20231222221236099](images/attach-ca7705ffa3292a4fbb01a3481ff28c3ef406e39c.png)
...
![image-20231222221534305](images/attach-4b6a60f8474fa76bae62ae4f36bc430fcf208440.png)

（4）因为我们使用的jwt，所以权限相关的设置肯定在jwt中。我们抓个包拿到jwt解密看看

![image-20231222221823154](images/attach-514876e12025381d5799788361e0d989c7ac5113.png)

![image-20231222221925066](images/attach-369cc335d08225fa0221b853f1e6cd83bd4ac948.png)

（5）把`wiener`修改成`administrator`

把第二部分payload拿出来base64解密，然后修改，修改万再拼接会jwt中

![image-20231222222217488](images/attach-58bd525d2995c3acdeabca4a6344f6b08a90a497.png)

![image-20231222222319473](images/attach-92eb0a698a6c2bf7c2537d688f1115fbbe78a9e6.png)

![image-20231222222404181](images/attach-3e2fa2caeaea5a5fa6f95c7f58c4f484293c5b48.png)

（6）把修改后的jwt替换原本的jwt

![image-20231222222504138](images/attach-d3ef02f32129a1a965ac3cd348689c9741219158.png)

（7）访问`/admin`，删除`carlos`用户即可通关

![image-20231222222553594](images/attach-d407635e1c8e86a332b0b15b47a1e72227eb8fe7.png)

![image-20231222222634903](images/attach-054474216e6bf1b634aeea1f31f6611de7b4bf7a.png)
...
![image-20231222223126751](images/attach-dad62dd7814f45a13840435458d9970cb5e83961.png)
...
![image-20231222224611748](images/attach-59f4c9ac0e06300046dfb445a71e4927323a71e9.png)

（1）打开靶场 --> 登录 --> 抓包 --> 拿到JWT

![image-20231222225235639](images/attach-fb4b3a855b4c0cfe1f583f9d4a7104be155e0703.png)
...
![image-20231222225046915](images/attach-109d5ee51bb9073b9711bd634cf550508fe4af16.png)

![image-20231222225746340](images/attach-a494896184581b90bae3bacf2a4e5f5462810a59.png)

（4）拿到爆破出来的秘钥`secret1`，我们篡改jwt

![image-20231222230033259](images/attach-c244470820f83392f7ffc09dae811e9e1dff7f1e.png)

（5）修改本地或者抓包修改jwt都可以，然后访问/admin，删除`carlos`用户

![image-20231222230440373](images/attach-a259bbce2db28d21413392bd03b58b9523262073.png)
...
![image-20231223025921262](images/attach-ccd88458698428bc78c07d337017665c6b98e836.png)

（1）这个案例我们使用`burpsuite`，首先先去安装一个插件`jwt enditor`

![image-20231223030051079](images/attach-5e930114c1e3e3ffb08192c7f3a946922088df7c.png)

（2）安装好了后我们使用它生成一个新的RSA密钥

![image-20231223030327718](images/attach-d7ce3815d8c37e6768c5128a22527b152ecd8a85.png)

（3）打开靶场 --> 登录 --> bp抓包 --> 发送到`Repeat`模块

![image-20231223030946972](images/attach-be06ae25a6f661efbd9dd397ffa0a0ffa949af1b.png)

（4）将sub内容修改为administrator

![image-20231223031024846](images/attach-1e40d298d98ccb08d0ebc8688b79dc4d0559b067.png)

（5）点击"Attack"，然后选择"Embedded JWK"，

![image-20231223031129929](images/attach-0901155ea34e5bef61a9926aa839711e79ea4f38.png)

（6）出现提示时选择您新生成的RSA密钥

![image-20231223031203432](images/attach-ded5d98df6af9d8f3bf4249e3276adc062072065.png)

（7）复制新生成的jwt

![image-20231223031245153](images/attach-01f22dbe3e9f69933ed94a613100a6b29764afc2.png)

（8）替换本地的jwt

![image-20231223031352592](images/attach-3d0344c8064310afdc3a96b6e1cf6347b6106919.png)

（9）然后访问/admin，删除`carlos`用户

![image-20231223031452606](images/attach-cd2648276a46a352174b552aa3a480f3c92a5781.png)
...
![image-20231223034620634](images/attach-cde495761d041254562b07d4d7bc6a41d57e98cb.png)
...
![image-20231223033528304](images/attach-b8e69cfbc3b6d0286b4c3509fb12eb5f4df1b86b.png)
...
![image-20231223041017444](images/attach-3b5a3c7871dd8d33b85c56fa2ff63ad0c778b60b.png)

（4）在bp中需要改动三处

![image-20231223041438999](images/attach-4039610c82aa783435eb4610a046c1307947bc23.png)

- kid：改成我们插件生成的公钥中的kid
- jku：添加jdu字段（漏洞利用服务器地址）![image-20231223034301655](images/attach-45591df776078c96b1572fae5303ea4c280ff1d9.png)
- sub：讲wiener改为administrator

（5）点击下面的sign，选择Don’t modify header模式

![image-20231223034459509](images/attach-f1eaaf0cdd74c6219aaa6e25f832fe36ac75659c.png)

（6）之后的流程

替换本地jwt --> 删除`carlos`用户

![image-20231223041239306](images/attach-cf9391c9128751c5d1df72209cac1d6783bbdf82.png)
...
![image-20231223163130615](images/attach-788d897f740206b1b7bfe734e430fd294ab0a6e8.png)

（1）使用bp插件`jwt enditor`生成一个 **Symmetric Key**，也就是对称密钥，并将 k 的值修改为`AA==`

![image-20231223162359656](images/attach-0def63c0b39b5c3c872a56b5b0c83480340fb2ed.png)

（2）打开靶场 --> 登录 --> bp抓包 --> 发送到`Repeat`模块

（3）接着，我们在抓到的包中修改 kid 值，尝试用目录遍历读取`dev/null`此文件。并将 sub 修改为 administrator

![image-20231223162825481](images/attach-1edbf54d62fdbdbd3d76ee8112fbe49ff6c0b6b3.png)

（4）点击下面的 Sign，使用 OCT8 的密钥攻击。

![image-20231223162857397](images/attach-abe4a30bd2a219d817d82fe07499f84868843d3f.png)

（5）接着就是：替换本地jwt --> 删除`carlos`用户

因为是目录遍历，所以多尝试。

![image-20231223162953206](images/attach-5c8b06f3f9652922585d2303a9b38fe1cd8d83b2.png)
...
![](images/attach-a94491d95a00bf43ba6ca57fe7270f9c6271a5b1.jpeg)

其他算法，例如 RS256 (RSA + SHA-256) 使用“非对称”密钥对。它由服务器用来签署令牌的私钥和可用于验证签名的数学相关的公钥组成。

![](images/attach-be23da66bbcd954fb0b958694bd8b56ed37dfd3f.jpeg)
...
![image-20231223172704453](images/attach-b1e25cffca92e79dc360c0ab2007f3e81541f52d.png)
...
![image-20231223171250162](images/attach-937a1e9baa1fc226811e52da333f9941dfa422dd.png)

（2）将公钥转换为合适的格式

在Burpsuite的`JWT Editor` 中点击"New RSA Key"，用上边获取到泄露的JWK而生成一个新的RSA Key

![image-20231223171605944](images/attach-47dc0f4b3c3e71905da7105852ec741ed5a5f93e.png)
...
![image-20231223172157829](images/attach-ded1346de1c680585d420ca4f01dbad43e632024.png)

（4）篡改jwt。

![image-20231223172427569](images/attach-afe9b647c9d063b539057dd5b0ed041b970165cd.png)

（5）使用这个公钥签名

![image-20231223172523042](images/attach-31874af231d266601b0b198e01c3b4e7eb4c120f.png)

（6）替换本地jwt --> 删除`carlos`用户

![image-20231223172605282](images/attach-44784c2242650ce02604611aab84c6d0161bc36f.png)
...
![image-20231223182733413](images/attach-b8608aa9f55855fa65e3146d4a0a380b89db73ab.png)
...
![image-20231223181749611](images/attach-34237b478588b5ee33d7a4ad52faadebcf1eea68.png)

jwt \_ forgery.py脚本会输出一系列token的存在情况值

![image-20231223181803810](images/attach-147b5d1b7c18d30a2b1c5a2f9215ee4c0e2d2463.png)

这里我们尝试每一个Tempered JWT，不过靶场这里给了提示说是X.509 形式的，所以我们只需要将X.509形式的JWT进行验证即可

（3）剩下的步骤结合上一关的一样了，创建恶意的jwt --> 签名 --> 替换本地jwt --> 删除`carlos`用户

![image-20231223182245303](images/attach-d56827420cbfe2c905a7da39785d0e5a6001b338.png)
...
![image-20231223183847582](images/attach-cb21e3311a76c21359f9288c5aeae7754505a311.png)

我们访问该页面并抓包，发现api:/square/GetAllSquareUser，用于获取用户信息，包括用户名、密码、邮箱地址等敏感信息！

![img](images/attach-75f52eb43994dceb8bd83e9ef945b74568da5ecd.png)

我们把这个数据包放入webfuzz模块中，删除他的JWT token，然后发送数据包，我们可以发现我们仍然可以获取到用户信息！

![img](images/attach-f6c7609728a8bf5be231e81e66260cc2cd354664.png)
...
![image-20231223184051843](images/attach-2599500b413225291678a8c860dc9afa96826d91.png)
