...
> **来源**: https://forum.butian.net/share/2573  
> **文章ID**: 2573
...
![image.png](images/attach-308054fbf605455e50dd0a5d90efc9becabe93d3.png)
...
![image.png](images/attach-22a0f85169c801ce7ce4173ae2385013f6a52a80.png)
...
![image.png](images/attach-a611c4a7c27dd6ee1452b30e3dc7dd07c43387b6.png)
...
![image.png](images/attach-58cc01bf58a774057db4714a6fbb1fa1d996e3bf.png)
...
![image.png](images/attach-cd159bb06d295d3a51f7b70283c0c7bd8195f31a.png)

注意exp的XML内容要写在SOAP的前面才能正常解析利用：

![image.png](images/attach-c5010af2d522d5f9621ee85d36c6a5cd307fea65.png)
...
![image.png](images/attach-f1a3f63425e510a9d09c205100a197e9b5f888e8.png)
...
![image.png](images/attach-be0aef3e057482e49f08fa267493fd02798cc14d.png)

2、PHP环境下读取文件

不支持netdoc协议，可以使用file协议读取文件

![image.png](images/attach-7fe1d10889e572a4515a5b860a860f5239f80437.png)

3、读目录

Java环境下可以使用file协议读取列目录，和ls作用类似

![image.png](images/attach-99db9c906895c502a15aadcf0d0a458de0d741db.png)

PHP环境下，使用file协议无法读取列目录

![image.png](images/attach-d19d0e6962ac2db434892df9bf44bdd2b53816ef.png)

5、绕过特殊字符

在 XML 中，字符 "<" 和 "&" 是非法的，所以读取一些带有特殊字符的文件时，就会报错“元素内容必须由格式正确的字符数据或标记组成”。

![image.png](images/attach-b01be125d1340b3873049a6c759085335fc78ab3.png)
...
![image.png](images/attach-2cc943d54ab034d1dd34894e64b9168770b082ac.png)
...
![image.png](images/attach-e98e3f47aec1160a23119105d164352fbe0d5be9.png)
...
![image.png](images/attach-b70fbcddf36b97cd964700e33de5435b931f6978.png)
...
![image.png](images/attach-14154bf5a344878e2cfd3f103d601186a54726aa.png)

可以看到在服务器日志上面出现了我们想要的内容。

![image.png](images/attach-e594d21b8bbe34ecb13c9c06493366781e6811f1.png)
...
![image.png](images/attach-8695b595760bca72a81bd9f4440d054029082659.png)
...
![image.png](images/attach-4a9fac6f473314e56fff9d235f75ee95289bff9d.png)  
**下面是一个探测脚本的实例：**
...
![image.png](images/attach-de61d351c060ca75a5800bcc1e61ab83aad9283c.png)

如何找到上传的地址呢，使用一个不存在的文件就能报错显示路径了

![image.png](images/attach-d0979e8dfb12040732dea2590e72d3d05e39553f.png)
...
![image.png](images/attach-fe6b97059aae0d3dfd16cd8456f133924c031ccb.png)

并且SAMLResponse可控，攻击者传入构造好的xml代码，造成XXE漏洞。

![image.png](images/attach-b1bf5de94cf1e1e3397d6808a03b9ca9028e50a7.png)
...
![image.png](images/attach-f667781573be98d2c579a9273e760e4eccad1a8f.png)

我们写一个测试方法，调用xmlToMap方法，发现成功读取文件内容。

![image.png](images/attach-ece4848c55bc6e7cb0f062a32a87ad159b8f4469.png)
...
![image.png](images/attach-a6758b8338f126f3e9e049328c89187f6872793c.png)

可以看到子节点都被转换为map输出出来。

![image.png](images/attach-03557163e0e400a1c6a3ac6a547eb32a1158d54f.png)
...
![image.png](images/attach-af70b794b2ec3a2e60082c4c20018513835b6f69.png)
...
![image.png](images/attach-6310e0d36e2bfe3f6410ea0645d61739ad71463a.png)

2、这个appid可以定位到application/admin/controller/PayApi.php的WeChatPayConfig方法中

![image.png](images/attach-c57d081b9ccacc7cb9f9c47578af72d1bebb4fae.png)
...
![image.png](images/attach-a4bd95b1921a92b9bba247741b0fdc3d383b2735.png)
...
![image.png](images/attach-aef7ab441739190f2709649a9bd3f6af8e4fc418.png)
...
![image.png](images/attach-8927efbf73da0e5dd921f912ab7fa27ba82d3cac.png)
...
![image.png](images/attach-c096522799f2dbf705da4daf84167b0f6ad57efc.png)
...
![image.png](images/attach-2dee752ac50bbdade71caaed99f34192e28cb2ca.png)

测试时支持上传和预览docx，xlsx文件的功能点都可以进行测试，QQ邮箱和网易邮箱的附件预览处都曾出现过XXE漏洞。

![image.png](images/attach-8e5108dd9f30cc7ec0038e4e548c3f465cb8ed9d.png)

修改xml文件内容，DNSLOG探测

![image.png](images/attach-3eb840c1afef1667f807bd7fcf48cee79430198f.png)

在网站上传功能处进行上传修改后的xlsx文件,dnslog收到信息

![image.png](images/attach-0f7457c614d04f7c2d716ce2b6d0ac78d4f1f841.png)
