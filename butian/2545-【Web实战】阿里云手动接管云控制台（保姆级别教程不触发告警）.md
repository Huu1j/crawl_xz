...
> **来源**: https://forum.butian.net/share/2545  
> **文章ID**: 2545
...
![图片.png](images/attach-aa95e17ba300ad9f53196ac0b49658a665d36ab1.png)

他这里有个在线调试的模块我个人感觉很不错，可以清晰的看到调用结果和返回的数据

星号是必填参数，其余的参数他也标出了是什么功能，我就不再赘述了，这里是创建了个molitest的用户

![图片.png](images/attach-3b3d46b6105a74cb427b303976c2c4b18a4f6c19.png)

创建用户之后给他加一个能登录web控制台的权限，因为登录控制台肯定要密码，所以这里指定一个密码

![图片.png](images/attach-720534856c909afd1e59729982a624980368f305.png)

然后我们查看一下云账号的别名，这里记一下，后续使用RAM用户登录控制台的时候会用到

![图片.png](images/attach-5b1e19b41f4df9e2fb0d77b079098a12fffe8436.png)

然后我们查询一下所有RAM用户，确保账号添加成功

![图片.png](images/attach-e887627ca8c2785083de576f4cfbc5a8a78dab63.png)

这里可以看到已经添加成功了，尝试去登录控制台

这里我们可以看到两种用户名登陆的办法，一种是用户名@《默认域名》一种是用户名@《别名》，这里的别名也就是我上文中提到的云账号别名

![图片.png](images/attach-959f28e6fbcdc5a9a4cbd29045eb055c94d1f441.png)

![图片.png](images/attach-28ba8913eb8ed354a2ba5329310402c0232c4eb5.png)  
利用用户名和刚才查询到的别名登录，输入密码，可以看到登陆成功

![图片.png](images/attach-0175e0ce4b795c00a93789c0fcd2ce339d313469.png)
...
![图片.png](images/attach-040f53984f37005c0ec7b9528dd0d05001269e4c.png)

所以我们要给这个molitest用户进行赋权，在这之前，我们要先查看一下权限策略

![图片.png](images/attach-6354377042abff794cd17c36e95153aec7b004f6.png)
...
![图片.png](images/attach-44bb4aec0cf324fe57a32cff61db733c194e64c9.png)

返回Requesid就是赋权成功了

然后我们现在来调试一下，验证我们是否赋权成功

![图片.png](images/attach-9511ee262f50b9363567c23dabfdf0d61dd75ede.png)

这样我们的账号就拥有权限了，我们再去登陆试试，

![图片.png](images/attach-0ceb709ce195877dfbd2d215e35b3b77a248ff8f.png)
...
![图片.png](images/attach-a44d237a84bfce37c2ef6cb854f9811696166421.png)
...
![图片.png](images/attach-13061e45e10807a86d281976d89fcd7978a91f62.png)

输入 `aliyun ram GetAccountAlias`查询账号别名

![图片.png](images/attach-f2bf0862e2c9ff011f9974828c597622a8f5b161.png)

接下来创建账号，输入`aliyun ram CreateUser --UserName molitest111`

出现下面这种返回值就是创建成功了

![图片.png](images/attach-62a5e80997e03016afc5b095b7fac441fbe71e33.png)

然后调用`CreateLoginProfile`接口给这个账号添加web控制台的权限，并加上密码

`aliyun ram CreateLoginProfile --UserName molitest111 --Password Poker安全`

![图片.png](images/attach-fb386f100c5fd0ab5c458dc3cd0f2eea05483dd5.png)

然后`aliyun ram ListUsers` 查看用户创建成功没

![图片.png](images/attach-877f06b6b7d65daf6d79939d22b9fac2d0630a42.png)

添加好之后，接下来调用ListPolicies接口 列出权限策略 给刚才创建的用户加上权限

![图片.png](images/attach-4ef04b93bce864c4d09d14bcdc7dba09567a2a8e.png)
...
![图片.png](images/attach-58d3daaf9723c065565981df865d37e97f780288.png)

如果命令实在是不好记 可以直接用aliyun ram，会列出当前模块的功能（太多就不截全了）

![图片.png](images/attach-a197c7bc2f3e56aac8a68a4a663adb7ad8672bc4.png)
...
![图片.png](images/attach-afa4ab4e709fc001b65da045c10c3d23ff7a1e93.png)

然后删除用户

`aliyun ram DeleteUser --UserName molitest111`

![图片.png](images/attach-f8aa87bc35ab9d846ad4d9f1044ea215c3d6f422.png)

再次查询用户，就不存在molitest111了

![图片.png](images/attach-2d036706beadc6a4fe927a82e04c89b0ccb35ca9.png)

到这可能看到我这有个crossfire的账号了，想必大家也不陌生，正是cf一键接管的默认用户，这个账号是我利用cf直接创建的，产生了告警

![图片.png](images/attach-829900f3f6a44893efad1450947544ec311a0548.png)

![图片.png](images/attach-15f3dc73707f08dab4a47bfdd19730541ce189ae.png)

那么相同的，刚才利用cli工具完成了接管控制台的功能，产生的动静相对较小，只会显示最低级告警，不会出现这种紧急的程度

![图片.png](images/attach-1e7a2d46fa5dfc382c2c8a2c7811dd9fc5fc9bc8.png)
...
![图片.png](images/attach-a11db1cd9f8e0d7e0a5f4ea20e77673a578b8573.png)
