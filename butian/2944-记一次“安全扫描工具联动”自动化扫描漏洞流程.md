...
> **来源**: https://forum.butian.net/share/2944  
> **文章ID**: 2944
...
![image.png](images/attach-6e756011baff873557122407e4217c5f4893e4d6.png)

选择`-->`让AppScan自动选择端口`（这里自动选择的端口和地址就是Awvs代理监听的地址和端口）` `-->`本地`-->`我需要配置其他连接设置`-->`下一步  
![image.png](images/attach-9fe8eefcc84b9ee34e35582d99b97035ec8745bf.png)

选择`-->`使用定制代理设置`-->`地址为：`127.0.0.1` `-->`端口为： `8083` `（这里设置的代理地址和端口就是Yakit的代理监听地址）` `-->`下一步。  
![image.png](images/attach-18e15563461f475e1cd23319ea70ce633faf556d.png)

不用设置，直接下一步。  
![image.png](images/attach-672a9d79156592af139e81407f032392fddcfc4d.png)

不用设置，直接下一步，然后点击完成。  
![image.png](images/attach-b646dc5af912ae2025b802a8cccc3480b35f182d.png)  
![image.png](images/attach-88dfad86b97ce4be9df4bc9e96dc4e1b96ea701f.png)

得到一个外部流量记录器，等一下流量会从这里走过显示出来。  
![image.png](images/attach-87ee305dcc6945e501be4bb5f8017887c25756c2.png)

### 2、设置Yakit工具联动准备

启动Yakit工具  
![image.png](images/attach-922466e23839c5047f3bb0fc94bd00d313e395e0.png)

打开一个临时项目  
![image.png](images/attach-6168453f1b74543ef3ead70e6c05b8e72c73955f.png)

选择渗透测试工具里面的`-->`MIMT交互式劫持  
![image.png](images/attach-79098719afeda00cd588616344a7396dc19f675c.png)

在这里提一下，Yakit默认下载有的扫描插件就15个，想要被动扫描的漏洞比较全面一点的话可以去插件商店下载你所需要的插件，可以一键下载全部的插件，不过扫描会很慢，下载一些你需要的就可以了  
![image.png](images/attach-a4f4ddddc1cad9d6ca7d89501eea19051ae2710a.png)
...
![image.png](images/attach-be2121077125c0cea6f2229fd5e453a30d505f0e.png)

后面扫描出来的漏洞都会显示在这  
![image.png](images/attach-6f9acd01cf28bafea434345cfb9927dffceee04d.png)

### 3、设置Buro Suite工具联动准备

打开Burp Suite工具，选择`-->`临时项目`-->`下一步。  
![image.png](images/attach-fd1050723210c9632b6475b16c4adbfe154f9759.png)

使用Burp Suite的默认值`-->`下一步。  
![image.png](images/attach-7ef02793a949486774470683a8ccd1fe8f55d15b.png)

选择设定  
![image.png](images/attach-91e2eab7f3305c2b2ace48a205324e46a8d076b9.png)

设置代理，绑定代理端口为：`8080`，绑定地址为：仅限环回`（这里设置的代理监听地址和端口就是Yakit设置的下游代理地址）`。  
![image.png](images/attach-ccf846993d13f9b0764a129d3b37020a9d9f5c3e.png)

设置burp suite的上游代理，目标Host为：`*` `（允许所有目标主机）`，代理主机为：`127.0.0.1`，代理端口为：`7777`。`（这里设置的是Xray的监听地址和端口）`  
![image.png](images/attach-1f9635aa0b29b43ebf90f57b7a7a2435941ef824.png)

新增实时任务  
![image.png](images/attach-7e9013d7d9848a504d83ff8456ff83292c5fa292.png)

设置被动扫描所有经过代理的流量  
![image.png](images/attach-826ab5e0e705f6f5563d528c6beb3c5bb9bc017e.png)

编辑内置的扫描行为。  
![image.png](images/attach-0d60b0f11474e804a0def9c7929b30a9d3dc64fd.png)

设置扫描类型，全选，火力全开，点击保存。  
![image.png](images/attach-71f23fba41eebf593145774843c2335773b26dbf.png)

点击ok，设置好了被动扫描。  
![image.png](images/attach-661df0b92752ceae06ca0e37a4bed4ee4fa56d96.png)  
![image.png](images/attach-2c32824e074f0a5915f8ccae237ba56e915bcbf4.png)

### 4、设置Xray工具联动准备

使用Xray监听`127.0.0.1:7777`端口`（这里监听的端口是Burp Suite设置的上游代理）`，被动扫描漏洞，输出漏洞结果到`123.html`。  
![image.png](images/attach-20f03e6979b285dbaeb4821f43c3d7d93a0d130f.png)
...
![image.png](images/attach-a24d39d328855596bcecdd1963bd491eeae1b654.png)

![image.png](images/attach-c298f6d9a023474b667352ed921be4ef0948cd6c.png)
...
![image.png](images/attach-0835db2cc26c4047dc1c7f0f0035c1c8dc004113.png)

设置成全扫描  
![image.png](images/attach-edce13c825800c1b188692ebbb6b8bfd4e2d1834.png)

拉到最下面，设置代理服务器，地址为：`127.0.0.1`，端口为：`50438` `（这里设置的是Appscan第三方客户机代理地址和端口）`，设置完以后点击保存，再点击扫描。  
![image.png](images/attach-4672aa7ddfe6ca38c6233930654a2c390b6dd074.png)

会有个弹窗出来，选择受影响的项目，创建扫描。  
![image.png](images/attach-beb4edc97e6a692dc4da2ce8c947f47fba7ff0a3.png)
...
![image.png](images/attach-81b76027fcd7aba78403736ae6aea71311c73e95.png)

**查看Yakit工具经过的流量**  
![image.png](images/attach-234f0c41c1063c49a6e35e5e533f5394572f30c8.png)  
放行劫持的流量，看看Burp Suite工具有没有劫持到  
![image.png](images/attach-25170b161f65c83b051c074efbc3128101787b76.png)

**查看Burp Suite工具经过的流量**  
![image.png](images/attach-42fbb97d2345b24479574289c9817fb954b1216a.png)  
流量过来了，继续放行，看看流量有没有经过Xray工具让Xray工具对流量数据进行被动扫描。  
![image.png](images/attach-8b884c3c3ee59455b46ce8c2d72134ebcd43288c.png)

**查看Xray工具经过流量**

流量经过，开始对流量数据进行被动扫描  
![image.png](images/attach-726b62c76e38b33a8604cfbe2fee6518de1f621a.png)
