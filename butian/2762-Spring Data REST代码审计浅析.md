...
> **来源**: https://forum.butian.net/share/2762  
> **文章ID**: 2762
...
![image.png](images/attach-79411a0c38de21aa84dd130270c00c7293def5e1.png)
...
![image.png](images/attach-f86091a6aa0d1d66c56ada89ef421c0828c18dfc.png)

 前面的流程跟SpringWeb类似，经过一系列处理后，会在getHandler方法中，按顺序循环调用HandlerMapping的getHandler方法：

![image.png](images/attach-e05f2ea8ed9fd39e37c04716e65b13dc0d84ab8c.png)
...
![image.png](images/attach-98fcf3d9e8f9bddb408a98a986541e948ff3b5db.png)

![image.png](images/attach-e350ca3bf89336e12f58779735f97e0ba8d39d68.png)
...
![image.png](images/attach-0d41e3c4438abd1fc76ebd38ee4ce3cffe07e48b.png)

 首先在RepositoryRestHandlerMapping#getHandler方法中通过getHandlerInternal获取handler构建HandlerExecutionChain并返回：

![image.png](images/attach-50ca054226acef47bdd120a9691c9c9f725b8fb9.png)
...
![image.png](images/attach-0eb701b852fe6b9c98a1c999c57380aa65881e22.png)
...
![image.png](images/attach-12fceebb89da7578cacb5038c265c0286dee61a1.png)
...
![image.png](images/attach-740fb0feae4e83c816287cacf249edbe5252e736.png)

 在BasePathAwareHandlerMapping#lookupHandlerMethod方法中，首先从请求头中获取`Accept`的值:

![image.png](images/attach-e7782e9bd9579a0143476c46976a18e11466c7ae.png)
...
![image.png](images/attach-e6e061a653a8b49597161549b0bb09f36e5949c9.png)

 例如当前的ReuqestMappingInfo对象如下：

![image.png](images/attach-fe81d05b579828bc0cbfcfd5a79db7362c2778fc.png)
...
![image.png](images/attach-68ed369ff1bea5149fd37d475428b194cf936937.png)
...
![image.png](images/attach-f56729f74b76de220287837bac277cb9613dffab.png)
...
![image.png](images/attach-a4cc6ee48794f80dcc020267c7d547ab30a8886e.png)

 然后会调用getRepositoryBasePath方法：

![image.png](images/attach-cc4ca9eb08f47f8f237eff04d8c03d78efc57222.png)
...
![image.png](images/attach-75a11ec46a0b7d43549dfdffd8d48591bd1f9750.png)
...
![image.png](images/attach-367ff9addf2599daec3dcab67efae24765ecc97a.png)

 这里首先使用 Pattern.quote 对匹配的字符串进行转义，然后通过正则进行匹配：

![image.png](images/attach-91254971f86fdcdbea6ed1ffe98986b669140199.png)

 若匹配失败则返回null，否则继续调用exposeEffectiveLookupPathKey方法进行处理：

![image.png](images/attach-75c3aeb24a6b6c95174f17e62dc4831f8da10469.png)
...
![image.png](images/attach-10aa1ae5e626efdbc0abcd5ba78a33126cde96cc.png)
...
![image.png](images/attach-42333572d8e618a028b7b34a6f13346065bff42d.png)

 首先会调用checkExecutability处理，实际上就是匹配调用的方法，例如案例中的是findAllByIdCardContaining：

![image.png](images/attach-2aa07b94da500bc0f74446a48ca0d79bb2dedbde.png)

 这里首先会获取所有的SearchResourceMappings：

![image.png](images/attach-a7757c5459654433efbdfe52b0788d88756c4ef7.png)

![image.png](images/attach-c6922fb972cfac72e6be0b359fd3ca7c351070f7.png)

 在获取时会对@RestResource注解进行处理，这里可以自定义访问的path：

![image.png](images/attach-94921ba06a2fa61ec29ae7e79ee188e7de47f8fc.png)

 若当前的resourceMappings都是非暴露的，则会抛出异常，否则继续调用searchMapping.getMappedMethod进行匹配：

![image.png](images/attach-d1ed899769818883bc99ee0af75705385a8607a3.png)

 在匹配前会先将对应的path封装在org.springframework.data.rest.core.Path对象中，这里cleanUp默认是true：

![image.png](images/attach-fe2224454d076e4bfbdd1f0a6fb9f8daa31b9d5e.png)
...
![image.png](images/attach-47a3f7be3d46b4279c5d52833ca5e4bf91bcb40f.png)

 然后进行匹配，匹配到对应的Method后进行返回。回到Controller的逻辑调用executeQueryMethod方法进行处理，获取对应Repository方法操作的结果：

![image.png](images/attach-24a30dbb123a870d31ba9b8f2d85faea2f491b28.png)
...
![image.png](images/attach-7eb9bc2965e78b41a93469df084b8c3339fbd6ac.png)
...
![image.png](images/attach-16f0414861990fa8742cfe1e93064cd04dcfb631.png)
...
![image.png](images/attach-8796c4da3bd38a2002046e4d676749b0e3a36efc.png)
...
![image.png](images/attach-9306c3d44907fbd2c3f35582ba3e304fca232326.png)

 这里首先使用 Pattern.quote 对匹配的字符串进行转义，然后通过正则进行匹配：

![image.png](images/attach-361b00e97ce4c4ef4358cef71e1aa2fccf0d2951.png)

 这里并不会进行URL解码处理，所以返回404 status。

 同理，类似`/{repository}/{id}`的访问也没办法解析编码后的`{repository}`：

![image.png](images/attach-f746701dd0180b30db754e712f2f9cf816e710bc.png)

 可以看到返回了404 status：

![image.png](images/attach-f5ad899f6b6ca1253f2d5d8b83d3cc3b8e1cb30b.png)
...
![image.png](images/attach-32e623a4dee44d46b0037897a3a1fdfb56a83dff.png)

 通过访问 `/profile` 路径，`AlpsController` 会提供 ALPS 文档，例如下面的例子：

![image.png](images/attach-35d082e8ef92489979f5a8dd1dc06b50d3bcb02a.png)

 其中具体的文档包含了关于支持的资源、属性、关系和操作的信息：

![image.png](images/attach-9fb21b41490f7d4bd9fa0d50669fb6f86518a54c.png)
...
![image.png](images/attach-2aba5f4b2c523a6a71a409a434150e19c88076bb.png)

 默认情况下alpsEnabled的值为true:

![image.png](images/attach-a3f0c9389d9cf93f7435cb1597b3d23a9b76f231.png)
...
![image.png](images/attach-5a3bcacb36074ae6abf4344d879d2e3732d40063.png)
...
![image.png](images/attach-7ab649de20ca7c127495466c9d40c0947423b4da.png)

- 在指定的方法使用@RestResource(exported = false)

![image.png](images/attach-32c26e5390e028be8d8ac56c46618062a96fbc1b.png)
...
![image.png](images/attach-1a4ba22fdc3156b161b695183c074dbff36490be.png)
...
![image.png](images/attach-c5d7ce59920cde186a2d880eb06eee5776dea56a.png)
