...
> **来源**: https://forum.butian.net/share/2521  
> **文章ID**: 2521
...
![image.png](images/attach-b6177a8ad1bbd6405dae911ab87877b21c562ab6.png)
...
![image.png](images/attach-499ef5d32ede807cb2d9a805bf0694ccfe4f0219.png)

### 第三方API

是由外部组织或服务提供的API，允许应用程序与外部服务或数据源进行交互,如高德地图API、企查查API、第三方支付API。

![image.png](images/attach-bd78ea3439d63fa494e79bd105940b0481ecc4f8.png)
...
![image.png](images/attach-0859f08beedafb2d66b9294e32f014bfa45d6157.png)
...
![image.png](images/attach-61e7426a833ccb8ac5309a54cd3b89c098311e68.png)
...
![image.png](images/attach-a5830dd9b0f3c9d72c7f43976f404b208795a575.png)
...
![image.png](images/attach-e483494df5ef37e5fef155c824300adfd10d1b37.png)
...
![image.png](images/attach-380b87f998707438855798b32067541d50af091f.png)
...
![image.png](images/attach-f3052ee7c8a715b5c29fd56523d33f26f640c656.png)
...
![image.png](images/attach-59e172e80dbcca4fbbf32702d05044ff86a026c8.png)

源代码中泄露了接口信息

![image.png](images/attach-a94d3cdf0f922eb1f2704a3982efcd889d55ef8a.png)
...
![image.png](images/attach-36c119a6de5c22b886868ea35bcaf837309a380e.png)

![image.png](images/attach-6595c1012133f3e6dcfe83048c7e9683515153e5.png)

当遇见不存在try it out测试功能时，或者仅可访问json文件，可以使用第三方接口工具进行测试

![image.png](images/attach-56ad11268f803692b0f959c7feda6b548881f2f4.png)

<https://app.apifox.com> ，选择导入数据，使用URL方式导入

![image.png](images/attach-46ef031f65293a7c874c2fb31421282446f7caa3.png)

即可进行测试

![image.png](images/attach-911a67749f40b3e2feda879f843c3a937b9450a8.png)

#### 其它接口泄露

除了swagger外，部分网站可能存在其它接口文件泄露，实际情况实际利用。

![image.png](images/attach-66f980e9bc3180352716eaa5bc102d0dee2f7822.png)
...
![image.png](images/attach-8b7b38e0358a0cf0c89287c71487865b43abc3e1.png)
...
![image.png](images/attach-ece3676c81ccc32e8e39f317cc13fde457d2b64c.png)
...
![image.png](images/attach-2fc4381345008460c201bbf78f00928f6b75a8f8.png)

案例1:

发现由Webpack打包的网站，然后直接查看原码，发现存在着直接获取系统所有api列表的API

![image.png](images/attach-9a7429c36c02d218889d37b94389d25eeb4ccbf1.png)

注意1:根目录api，直接访问lyapi.json是没有任何返回的  
注意2:api站点（8070端口）和网站地址（8071）是不一样的  
![image.png](images/attach-d00e29ddc5f23ec339085a5cbde5eeb6e9504dcf.png)

![](images/attach-3dea37cad4e202ecad55f.png)

利用第三方api管理平台进行测试

![image.png](images/attach-20ab0548e15d839909a537ef9c1b34a40a8360d7.png)

将服务的前置URL改为测试网站的URL

![image.png](images/attach-705d8956fb4e2c5ec34df9fdbbd2ff1246201a3e.png)

![image.png](images/attach-13f82aa91b235f69fba1f3d9fa23f5f3bca91ff5.png)
...
![image.png](images/attach-78d53dfc468bb9aede3d3c6a68437731cc8ad261.png)

使用graphql-voyager可以对数据进行解析展示：<https://graphql-kit.com/graphql-voyager/>

![image.png](images/attach-f03655004e09957d39a8bed46d1b72100d3aca60.png)

某真实案例通过GraphQL 内省查询，导致全部接口泄露，通过对接口进行调试，发现未授权访问获取大量敏感信息，并有接口可修改身份信息造成任意用户登录。

![image.png](images/attach-a607b774bcd1e7862d0291c9b4c729ea3be03615.png)
...
![image.png](images/attach-d5977d09665cc0439e5efe9606cf2e52f37eea6c.png)

### 参数遍历

案例1、参数未加密或者加密失效，导致可以对接口参数进行遍历获取大量敏感信息

![image.png](images/attach-00582a5e0ca8c890ac726b041efa22f418d87d77.png)  
案例2、其中一个API接口可遍历返回sign值

![image.png](images/attach-c4f7cbb86264a2f5936726e1cf1cda5374f46667.png)

利用sign值在另一个接口可获取敏感信息

![image.png](images/attach-cf359fa92f9d9b10b3e95184eae168c779a36385.png)  
案例3:paypal

1个价值10500刀的洞

![image.png](images/attach-e5c977dfb16cf5bd28e99ee11b10fcec8ae211a6.png)
...
![image.png](images/attach-16414516e47a33d30d4de11568e8f3b08c9aec7a.png)

直接访问API可直接生成Token

![image.png](images/attach-23d734c0977c919bd4797b0d44907510b0c80dfb.png)

利用token访问其它需要鉴权接口，即可获取大量敏感信息

![image.png](images/attach-2e118b2a29ece15ccf4edfebc3f2871cdf2db67b.png)
...
![image.png](images/attach-eb3550eb5c2131ec87cb45dab944c36e2c43c192.png)
...
![image.png](images/attach-49d8b516c0f09f519cd3d7c02e8e11c4f1a5a42b.png)

以管理员身份登录，即可看到千万级别的敏感信息，以“黄”模糊搜索为例，发现139211条记录

![image.png](images/attach-54b6c4217795efa5033e2ee40a002eb4f450f4c6.png)

### 接口易猜解

访问个人中心，可获取全部用户的ID

![image.png](images/attach-b70146baedd426acaf7e51eb63969d4487464cdb.png)  
...
![image.png](images/attach-60554733125fa0fd9935eac6b5b3e569f2655e27.png)
...
![image.png](images/attach-83e705c9cad154b9c613eab08cd3ff2b76a6aa57.png)
...
![image.png](images/attach-1830659825961e392d54867c371a526cd7297e3c.png)
...
![image.png](images/attach-1558253dd294f768be0e0197ff796fc183ec56c7.png)

案例1、前端页面仅有姓名、企业、性别、类型、参加工作时间5个字段

![image.png](images/attach-2c8efb1749f212fd8ab9789afbcba901457edf49.png)

但API接口将数据对象所有字段均返回，造成数据泄露

![image.png](images/attach-7c6ce1423c2923cea957dab3f189835561e3e316.png)

案例2、前端页面仅有5个字段，后端数据全部返回

![image.png](images/attach-8a4614f4b3eddaa0552aee2fa616ff63cd0c2591.png)

案例3、边缘功能处，选择指导老师，模糊搜索可出现姓名、学号、学院

![image.png](images/attach-476a0e52b504af4b3202554f96011b337a818f02.png)

但后端数据返回了大量敏感信息

![image.png](images/attach-af31e9ec461810d524d860b165892c4d05946034.png)

案例4:接口返回过多数据包含用户名，通过收集用户名，爆破密码，登录系统后台，利用身份认证获取未加密敏感信息

![image.png](images/attach-d32ba81e144806ff8107109c67bb2cdbd5b7b04d.png)
...
![image.png](images/attach-fc3dd7c5e2242f36311eadb5753907a872df0b0f.png)

但仅在前端进行了脱敏，后端数据仍为明文

![image.png](images/attach-987c9f6ed571b090b225529c6ea89ee1c000d6a5.png)

### 部分功能未加密

案例1、前后端均加密时，使用文件导出功能也会存在未加密情况

![image.png](images/attach-39e5cfd62f6042837cb8d866562769c2704edd5e.png)  
案例2、常用的主要功能页面进行加密

![image.png](images/attach-2b83d536518269fc191734222f9053edb7f8ebde.png)

边缘功能未加密

![image.png](images/attach-6bc48678711c9154b2f4cb735ee44ee4a8d4a571.png)

接口参数"mobile"加密 "mob"未加密

![image.png](images/attach-a439074c8eed5f5ee3841dd5ffc345f14199ed9b.png)

### 加密方式失效

系统对身份证号的出生日期进行了加密

![image.png](images/attach-6e8bb2efe4a528f6530fda0701c38b095cc50858.png)

但出生日期字段是明文的

![image.png](images/attach-83bdbbe312c7544c08d79624ff01d5b0816ea282.png)
...
![image.png](images/attach-8dc0859d350efcaf9d705809d26da6ef27e1840b.png)

在遇到参数较短时，可以对参数进行爆破

![image.png](images/attach-c7a3704d18e4211e4c7ece50a8ba2d166426dc76.png)
...
![image.png](images/attach-8194389a16bb32d85e9448418c661bddfeb0e831.png)

案例2、JS文件泄露账号密码

![image.png](images/attach-2c2bf2b1a5fc2638a34ddea478cd93a022131544.png)

案例3、JS文件泄露OSS存储桶密钥

![image.png](images/attach-602790e29743cba6c808369cbd45f96310ac0fbd.png)

案例4、备份文件泄露AKSK

![image.png](images/attach-02c61e7c990e9c983d2e94ef24209584409d016b.png)

案例5、JS文件泄露AKSK

![image.png](images/attach-c08c130ac26b0ebf880653ff87168ebd3305fc20.png)
...
![image.png](images/attach-c9320f23eaae673cfb539a9600ce5b6253bb6706.png)

下载jar包，查看配置文件

![image.png](images/attach-cd02b1b1822891fec99f833006c908f43c09d0fd.png)

![image.png](images/attach-5cf4a8e4d053960c0baf0a378e7031cc7a42b4a0.png)

案例2：Harbor仓库任意下载镜像文件

![image.png](images/attach-02ac8c65fa0f131dae691f82f39019c75844260b.png)

![image.png](images/attach-39325eea3d6d572dc1b21df201793bdb2addee72.png)

jar包反编译获取配置文件

![image.png](images/attach-082fbe2e8b0448189226da7d176aac49dfdeb3f4.png)
