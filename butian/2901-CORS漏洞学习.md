...
> **来源**: https://forum.butian.net/share/2901  
> **文章ID**: 2901
...
![图片.png](images/attach-8f9d21b933ec30a66445773390bbf58de8c4714d.png)

A网站有个接口，是返回用户个人信息的。先判断用户是否登录，如果登录那么就会返回登录信息，如果没有登录则返回未登录的消息。

![图片.png](images/attach-4a8ee5fb645c7dbcb0542e127515b1632360feae.png)

登录代码如下：

![图片.png](images/attach-ac23fc2ae566e95e06c5323d00b487ab7e741cfc.png)

如果登录成功，那么访问getinfo会得到一个json数据，登录失败则是提示重新登录。

登录成功如下：

![图片.png](images/attach-1036db3e8d3cb09725b6ce4e71792a8d4f5fe28e.png)  
未登录情况如下：

![图片.png](images/attach-562bb8dad1475242f24ac0d1cba6b5e906a0c84d.png)  
那么假设我们在a网站登录成功的状态下，这时候访问getinfo接口的时候返回了身份信息。那么这时候又去访问b网站。b网站是一个恶意网站，b网站的js会发送一个请求，请求的是a网站的getinfo接口。b网站的代码如下：

![图片.png](images/attach-ddd1900c273f008829508721143162ac0a30d7db.png)

很简单就是一个img标签，通过src属性对getinfo接口发送请求，那么这时候是不是会附带a网站的cookie过去，然后就可以劫持a网站的用户信息。示意图下：

![图片.png](images/attach-47505ddf5aa8b9c7c9680211b82f716dfd9fdf3b.png)

![图片.png](images/attach-65da9d656bf8e4e2d3efad6be4fe5858706403ae.png)

步骤1：登录A网站

![图片.png](images/attach-a4abe5f93a92ae57c2e7559e586f6746d04475b5.png)

步骤2：打开B网站

![图片.png](images/attach-284d2aaacdee1c71044764720161bc06c77aa0ed.png)

也没有返回信息。

![图片.png](images/attach-e04348453d8c53d10546fc2085d94f55f7c86650.png)
...
![图片.png](images/attach-dc6aaf064353da630d42a976f8e579d5dc931286.png)

执行效果是失败了的，pay.a.com并没有获取到数据，这个是因为a.com还没有配置CORS头信息。

![图片.png](images/attach-a5fd39b5f8363940afd38eff75d42a0ca654afa9.png)

在getinfo代码上配置一下CORS头

![图片.png](images/attach-b764d672da4de479f3953b1184948df21c611f30.png)
...
![图片.png](images/attach-bce21d47c5361eca4602115b54127bc1b0ac8d9c.png)
...
![图片.png](images/attach-51b4795154c716021afe62e8f4aade45d077fa85.png)

**注意：这种情况下其实也是不能成功利用CORS漏洞的。**

攻击网站b.com代码如下（就是把pay.a.com网站的代码复制过去，但是发现pay.a.com能够成功访问到getinfo接口，但是b.com访问getinfo接口不成功）：

![图片.png](images/attach-16afa821ab20a5abfa418a3acb8ba5ff6b0c46ec.png)

b.com攻击结果

![图片.png](images/attach-854407684883c2e726f14d2b3357e97343b1f548.png)

然后查看b.com向getinfo接口发送的http请求。从header来说确实是允许跨域的。

![图片.png](images/attach-e5d07ca77992f1f7c7f12b32a0f14d3ee9a00f88.png)

因为除了CORS保护cookie之外浏览器还有一个SameStie的保护策略，控制是否随跨站点请求发送 cookie，从而提供一些针对跨站点请求伪造攻击 (CSRF) 的保护。

![图片.png](images/attach-27f99fb64506fcfb77b7e74b605bfa54122163d8.png)
...
![图片.png](images/attach-bf4539a94860eb1cbe3fa5c3f9d123c7adb0405b.png)

可以看到b.com成功跨域访问了getinfo接口并且携带了cookie。

![图片.png](images/attach-de56c532c2686cbcd88b0df80e16df6e45dd9dae.png)

![图片.png](images/attach-cc1ce5fb1beebb34592b641640d74bf73e991287.png)
...
![图片.png](images/attach-307c5c17273bf7fe70cc722361e9dba7c833d17b.png)

原本的方式已经失效了

![图片.png](images/attach-4d822962d4f0250c7c633385afb60883784a1329.png)

那么就可以使用a.com.b.com这个域名去进行绕过过滤

![图片.png](images/attach-7af72e7a2e1ef0e77f3281c28f5ceccf3b5bdfa4.png)
...
![图片.png](images/attach-b0fc9198afd6baa8ce8b91eff079640b8370404c.png)

然后可以使用xss使得bug.a.com发送跨域请求。

![图片.png](images/attach-e9c7f7bc4e9511fe66eeb3fb9571673998226904.png)

然后只需要iframe标签的src属性变成xsspayload即可。

![图片.png](images/attach-35e188713a9bba12c904c826e97c70e7b447321c.png)
...
![图片.png](images/attach-f050097b7d2efe7f793d77c5ab74065e6cd4d3d4.png)
...
![图片.png](images/attach-2da9c12e1e2b0b432526de906d06fa7bbd161177.png)

![图片.png](images/attach-fefb1f8a965c6692a3b53ca10379a816e4131371.png)

发现确实能够跨域拿到数据，并且origin设置为null。

![图片.png](images/attach-132767e53ee55d4aebc317220ddf2390a7f28d7d.png)
