...
> **来源**: https://forum.butian.net/share/3030  
> **文章ID**: 3030
...
![image-20240516134124722.png](images/attach-cd33afc6b48b5690fa2ea43bfcb9c6c6c7cf21f7.png)  
而事实上，除了传统安全中的DDoS攻击、中间人攻击之外，以ChatGPT为首的大语言模型还面临着一种新型的攻击方法，就是越狱攻击。

我们可以看下图的这个示例

![image-20240516134432565.png](images/attach-be5e69ba60efa929d17f5de72f3338092a45dac8.png)  
...
![image-20240516134711318.png](images/attach-bc35e578fd00946849b7f7ed92979731eb463c60.png)  
...
![image-20240516135148472.png](images/attach-4a7f65aeddc1324a2093f57711a377067caa4dcd.png)  
攻击者详细阐述了这些问题，用如下的越狱提示

![image-20240516135209745.png](images/attach-cea5e3d36ab9cb526b475e7ca16bb04cb6c960ef.png)  
最后得到一个组合的输入集，如下所示

![image-20240516135225877.png](images/attach-39017f461de5c50907a6287ddbad90c54a671609.png)  
当这个输入集T呈现给大语言模型M的时候，它会输出一组响应

![image-20240516135311892.png](images/attach-709ec6edd0bd76f50bd5410cdedb0110d113ecb6.png)  
...
![image-20240516135518898.png](images/attach-18aeed66a0146d5577c6b21545dc321aae607bab.png)  
模型会估计下一个标记x(m+1)在词汇表上的概率分布为：

![image-20240516135548737.png](images/attach-9410fdafe309af984b0ae24340f35b1df29d6d6c.png)  
越狱攻击的目标是提示模型以产生以特定单词开头的输出,也就是如下token

![image-20240516135629182.png](images/attach-6f2cf19cdef73ca5f1a5ab12e81ca83176ba5614.png)  
那么给定输入

![image-20240516135708864.png](images/attach-f4eb8a36eda9b790cda3216e570ebb5cdd34feba.png)  
它的token是

![image-20240516135725261.png](images/attach-27f09575f17002f74e426798c1d9664e404d366b.png)  
我们的目标是优化越狱提示Ji来影响输入的token，从而最大化概率：

![image-20240516135809845.png](images/attach-2f08658ddf7b365dcc504cfb8db093ab0bde726e.png)  
以上就是越狱大语言模型过程的形式化表示。
...
![image-20240516140134416.png](images/attach-616afda903e85545ce0a9ebea7b8f2130eb78394.png)
...
![image-20240516140340089.png](images/attach-024a0491c54678654aa68aa5b2885f8e8bd0f1fe.png)  
我们可以直接使用一个函数来计算这种似然，来评估遗传算法中个体的适应度

我们使用如下的对数似然作为损失函数，即给定一个特定的jailbreak提示Ji,损失可以通过以下方式计算:

![image-20240516140415479.png](images/attach-6a944909dda5f869e0bbbadb06c9ebbd9971cba7.png)
...
![image-20240516140908738.png](images/attach-4db678d03ff0617750439530c3b69a18d5145481.png)  
...
![image-20240516141047649.png](images/attach-91d3ccd46073ef2bc94e9785b01c054b1dca4fa3.png)  
...
![image-20240516141156266.png](images/attach-12bbbd7a399036e051ed96d8b9304e0459b7e0d2.png)
...
![image-20240516141256924.png](images/attach-60b61b289bf6e7c81d43bb0051abfc042fb5ca81.png)  
从这个字典中，我们选择前K个分数的单词来替换其他提示中的近义词，如下算法所示

![image-20240516141318219.png](images/attach-8454aa5cf2dc1cc149581b0af7aaebca31d8ce0c.png)
...
![image-20240516142646300.png](images/attach-5f2b297aa181d37bc6a31009d2cefc533ba8aa1c.png)

![image-20240516142657492.png](images/attach-5d69f9d49a3cb5af96c6912126790cc9bc3aa90c.png)

![image-20240516142714130.png](images/attach-231c619a7c220e6b0c1bb13963c38b5c73b36a24.png)

![image-20240516142724101.png](images/attach-6152e55060e930890a842342a0faef7b5467fee7.png)  
这里先分析一下大概的几部分：
...
![image-20240516143059302.png](images/attach-d27584c2573ffe7652e7e852faa70717b6f98877.png)  
直接来看已有的结果，基本的例子如下所示

![image-20240516143232198.png](images/attach-e6e70fc35f6cfed208051e990e40d601f7df6ef7.png)  
我们来些越狱成功的

![image-20240516143441316.png](images/attach-9e100d66f8e54f7fc9d3dfc7c6e256d22ae35bde.png)  
上图中Passed为True表示越狱成功，而我们圈起来的第二个框则表示，输出的是越狱内容，这里就是告诉我们如何制作恶意代码

再来看一个例子

![image-20240516143553979.png](images/attach-0d4d7dc4bccb665f1d9140ec9b8db08e5ab33895.png)  
...
![image-20240516143706741.png](images/attach-45ee4968dc8c761864839294407837067d8af9fc.png)  
...
![image-20240516143834656.png](images/attach-4adeadfa3dec55efcf27f6e5af40ddd50eef384f.png)

![image-20240516143847439.png](images/attach-fdc99d219786f9535000c9b325cad9272d75aa43.png)

![image-20240516143859815.png](images/attach-fd2875675a570562fff21b7266f23d1c5c98dbda.png)

![image-20240516143910550.png](images/attach-e00c91403cc12c56acb458d959cf3c4c93afd12e.png)  
我们来解释下主要组成部分和操作：
...
![image-20240516144541987.png](images/attach-836bb79fc97965291581387507c36a4f521a124e.png)  
...
![image-20240516144707711.png](images/attach-f7bbeba0db7f71700a4bb47c70a36e3c6abf984c.png)  
...
![image-20240516144750081.png](images/attach-20dc3285d6f6aa8caf7f401fcc755ed93c82b746.png)  
这两个函数分别是：
...
![image-20240516144937891.png](images/attach-a391d42f6f88c0bbe4cbeb5d7b6bd4feacd417fb.png)  
我们去看看已有的实验结果

![image-20240516145022961.png](images/attach-32bbc4d9213fd9dc7f1314cb2ecbbe9bb91cc019.png)  
...
![image-20240516145149630.png](images/attach-dfeaa6e9c93354c4e8a310d3a4bbfd3bfc531830.png)  
如下的答案是教育小朋友怎么使用武器的

![image-20240516145316080.png](images/attach-1f52690b1de065146a2a0b4d2e39b325ef10a6ee.png)  
如下的答案是关于如何pua人类的

![image-20240516145420079.png](images/attach-2fd0042ae97dc533d4e48e6e0ac7481355a3ac7d.png)
