...
> **来源**: https://forum.butian.net/share/2661  
> **文章ID**: 2661
...
![image.png](images/f3599575a4c1d85b963f1d5569e5340a912b8f7303c57.jpg)

路由分析
----

在WEB-INF目录下，找到漏洞路径对应的服务名称为`CommonServletDispatcher`  
![image.png](images/f4872745bce6583e6c39edd199051d1c0cace602f2e75.jpg)  
继续浏览该文件，通过服务名找到映射的类`nc.bs.framework.comn.serv.CommonServletDispatcher`  
![image.png](images/f342297a7cb69f07d37af1d646916dec68bf7462ecad3.jpg)  
经过耐心的搜索查找，在`fw.jar`文件中找到对应的类文件。  
![image.png](images/f98166187323f23eb616539d33e6c090e9efeb848f559.jpg)  
...
![image.png](images/f18225914249992a65b29b9281e2d679870598ac2977c.jpg)  
![image.png](images/f464135bc8cf55290db531c5f7726372715d22d44158e.jpg)  
看看之后操作，还是在`CommonServiceDispatcher`中，存在`doGet`方法对传递进来的get请求处理。  
![image.png](images/f4992496c1e933b655db83f76fb3d1b59da4e8544d165.jpg)  
此处，就是前面的`serviceHandler`实例出场，调用`ServiceDispatcher`类中的`exeCall`方法对请求数据进行处理。  
![image.png](images/f29924838ab065e8f3883beb97cacb615b3e829ebc0a8.jpg)  
当然，还存在`doPost`方式，但同样调用`doGet`方法对请求数据进行处理。  
![image.png](images/f8007164492f6334a48fea8ff583a87fd49761c6a810e.jpg)
...
![image.png](images/f142309d5a318589b2ec5db58aaf5ccc5f8719668c584.jpg)  
...
![image.png](images/f140856f03c91a5694fe42afd19bd1aad17bda1c85f1b.jpg)  
这里肯定会比较好奇`NetObjectInputStream`的作用，其实它就是继承自`ObjectInputStream`实现的自定义的序列化类，因此同样存在反序列时自动调用的问题。  
![image.png](images/f731422618f2f5e06c4da3f150105fbb8627ae88665de.jpg)  
...
![image.png](images/f609444b66e54deb5b3e30b1ff2c6277464f4e785d206.jpg)  
![image.png](images/f4107778769be3a0fb32d1cfcaa6e557c8799d13f5718.jpg)  
在客户端的jar包文件中可以找到使用自定义的序列化类`NetObjectOutputStream`来写入序列化数据并向服务端发送。  
![image.png](images/f1525327aad90d0b5a593acf459b28a5373f87b5315dd.jpg)  
跟进一下`NetObjectOutputStream`类的`writeObject`方法。  
![image.png](images/f803270fc6cebf38461c5881bb10bec7bc6d779abdc34.jpg)  
...
![image.png](images/f1167869fa204054d050729c3c219aae7cb6ac2275187.jpg)

漏洞复现
----

编写DNSURL链进行反序列化点的探测，序列化方法使用上面提供代码即可。注意一定要引入对应系统版本的依赖包`fw.jar`和`basic.jar`，否则生成的序列化数据可能会无法成功反序列化。  
![image.png](images/f73771362befbb5ac78aa4b0df06942efb3ec85f33041.jpg)  
输出十六进制字符串payload  
![image.png](images/f126330e3cdd644b21eb51fd37aedc8a89c39a742ea3a.jpg)  
...
![image.png](images/f8953195a993750bb18d4ae13530b961f15b4c9605d38.jpg)  
![image.png](images/f6553823d79bf84ff2071b64f63832ffda7fcc06d806f.jpg)

### CC6链测试

该系统依赖中，发现有用到commons-collections 3.2.1，则可以通过CC链达到命令执行目的。  
![image.png](images/f5235730129c996d8620730d50eddb608ce795f78bc49.jpg)  
因为版本缘故，对多条CC链进行测试，成功利用CC6链进行远程命令执行。  
![image.png](images/f321287024331d2176c84d21d7f518bc274200177dcc8.jpg)  
![rce-gif.gif](images/f2011553a60dddf57501e49e5c4587a1a231d461ef2c8.jpg)
...
![image.png](images/f13675833176e5cafff5ef0d965e9f4b4d5198ffb1c45.jpg)  
回显的话，还需要收集框架或者中间件信息，方便确定回显链。这里看到该系统使用了Tomcat中间件。![image.png](images/f6206126f24ebb2d5fcf12f8a58ab7a1805e81792fa0f.jpg)  
...
![image.png](images/f6475164916851f6f26723a528446ae0b0c5d79810176.jpg)  
成功触发回显命令执行  
![image.png](images/f111472e07a91e9535e7904eaf0c65f0a9023c9f7322f.jpg)
