...
> **来源**: https://forum.butian.net/share/2875  
> **文章ID**: 2875
...
![image](images/attach-2845cfffc5c58a17079802ec0c4972410c739180.png)​​

![image](images/attach-61a3aaed6fc4c266f939963f7406b0b7c2ccae69.png)​​
...
![image](images/attach-c10ef892dcd83bb2c843fc53a036fe8e689e64c5.png)​

抓到登录的接口

![image](images/attach-37abf26632e5a629c7d44877d4becd675e1d36ed.png)​​
...
![image](images/attach-2150e137b3189d1221617836fd7ccf354d33ef83.png)​

‍

![image](images/attach-faa74a0faf9393183f02d86c3eca29db09b55f3e.png)​

继续跟进`\system\modules\user\components\LoginAction::className()`​

搜索一下`账号不存在`​其实就可以找到确实是这么个对应法了

![image](images/attach-e028e27fb9a6d6c229c755c8d36e6e5e5fa95f87.png)​
...
![image](images/attach-839cdec39ae8cb79cc96d7ca1e871f7efc2290ef.png)​
...
![image](images/attach-deb6037fee58ed2e213060e7760a15fa7039ba7b.png)​​

‍

当进去看上传逻辑的时候发现有一个很抽象的点，开发把扩展后缀的限制注释掉了，所以导致了后面写`$config`​的时候会没有效果

![image](images/attach-b2fa6fd37211e1b21eb041f0460674163b1bfad5.png)​​

那么很有可能就会存在任意文件上传了，然后下面的操作就是跟进了下`saveAs`​方法发现也并没有什么过滤

![image](images/attach-36bddd08729b14f492ac5c607a9e12ab9781d008.png)​
...
![image](images/attach-f7c90ac5466066105589071eb2140abc8e22c20d.png)​​

我突然看到一个点，他通过命名空间来进行调用方法的，所以说只要出现了`SaveUpload::saveFile(`​ (并且在modules下)就会存在任意文件上传了

![image](images/attach-eeb09c3b06e57bee42681b1f6b6dcc5439635c26.png)​​
...
![image](images/attach-70693efde62b2bf65ee3c185035fa909b46db437.png)​​

但是上述两个最为简单的发现并没有成功(也不是权限问题感觉)

‍

![image](images/attach-8a1703e637e04565e7fb36560d68cb96d9ffac3f.png)​​
...
![image](images/attach-1f91b7aeb31a6267a3123f50bab471a40cdff061.png)​​

而鉴权的代码是这样子的

![image](images/attach-69b9827fbc3812d3c8fbc985bea6793c877c8a26.png)​
...
![image](images/attach-2ac95e16d901403e18d1a3d20f9d494abef5ee97.png)​​

接着审计代码逻辑

![image](images/attach-74480b61d4d5cb1e3c4dafb8e1315cef6ce490a7.png)​

代码很短，可以很轻松读懂，获取一个json值，然后获取他的`OPTION`​值满足他的switch值就可以进入到上传的逻辑，可以进行文件下载，也可以进行上传

经过测试，可控

![image](images/attach-2776476cf93d53e6f80e2de61696255387c0cbcf.png)​

‍

![image](images/attach-ab4fc5f75e574c015240d4465ab3f5d3fed0d13d.png)​

接着就是构造下载包和上传包了

‍

![image](images/attach-a0bd47fe06071064450ed17f8d4d1c6d0ff6aa6c.png)​
...
![image](images/attach-ae2051a3212f42e9134b19d3229e752caddc87f7.png)​​
...
![image](images/attach-8dc2614c8c8d6596789a2e9bb66e063bd074d335.png)​

‍

![image](images/attach-cc8a1f82dc656d48347d056971f97f4e723aaef8.png)​​

当然也给了注释

![image](images/attach-564e970b423cddc7f44736f055358801117b2707.png)​​
...
![image](images/attach-bd4ff8b7e5ae831e168ba9c3723b47c20266416c.png)​​
...
![image](images/attach-61ab61e3e3d32449c2935bbc822d25d5f1cce33f.png)​

我的内心想法和审计思路： 在众多目录中当我看完开发文档中的`system`​根目录我去瞄一眼`api`​目录是一件很符合逻辑的事情，细看这里有一个这么写的代码

![image](images/attach-5cf31bddd637555d7afabce81427b7735d04a0ed.png)​
...
![image](images/attach-e816e0971b15801f0ad74e61b920b23ce9f2c172.png)​
...
![image](images/attach-5c9e8f3d07edfdd6197bf14b004161695946ea2a.png)​​

‍

这里因为他继承了`BaseApiController`​ 所以跟进父类查看

![image](images/attach-70e2d34fe1c5513e2444616f9f153c1a7068a5c2.png)​
