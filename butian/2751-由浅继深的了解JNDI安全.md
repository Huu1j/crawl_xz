...
> **来源**: https://forum.butian.net/share/2751  
> **文章ID**: 2751
...
![](images/attach-8f5b9b566f701b83c077ed1f1ca54e62d502a280.png)

跟进一下客户端lookup方法,跟进到RegistryContext类的lookup方法，从这里可以看出来，其实调用的还是RMI的东西。如果客户端的lookup参数可控，就可以让它访问我们恶意的链接了。  
![](images/attach-06fec7bb74b27b3a1eb50320932added11301ad0.png)
...
![](images/attach-cd21a41cb0b76b3dcf057bdb3badfe2966f91c44.png)
...
![](images/attach-785f1a272429d2ff390ecae5469dc0c950aa3a90.png)

到这里的调用栈如下。

![](images/attach-f02b6b1ad5438e9a3271556d65f70e9a946ed568.png)

decode之后就拿到了原先的Reference，同时在这个方法中调用了NamingManager.getObjectInstance()方法，跟进  
![](images/attach-641f3f7feff66409ff21acc66ac69efab74014c8.png)  
在319行调用了getObjectFactoryFromReference方法，从引用中获取对象工厂

![](images/attach-c486744e5106b65905392cc8e6d77f02e445e3c2.png)  
这个方法中首先调用loadClass加载，调用AppClassLoader本机加载，此时是加载失败找不到的

![](images/attach-fac90df2fdd7a1e4b4011b3c2ce9f43699a67969.png)

![](images/attach-dab932ed43d66f7f6d3abebc855c21c32556ba64.png)

接下来就是利用codebase查找，codebase就是上面提到Reference的factoryLocation，在调用loadClass加载  
![](images/attach-e3c565811d7a8c11ce55b7b99011e7c449e3de08.png)

把codebase传入URLClassLoader，利用loadClass加载  
![](images/attach-5ab9390dc6df9d1426a51a0afdc45e1bb204e837.png)  
这个里面就会对应初始化加载，对应URL下面的类。  
![](images/attach-d5616442967841c6f0ff345428e88fc9efe42876.png)  
找到之后newInstance实例化，执行完这一步就可以弹出计算器了。  
![](images/attach-dfc152a4d664598f8fffb2356e6e78e2fa734cac.png)

![](images/attach-8c6207e2d8d40abb2d9175abc9a6d5a5a9441ca3.png)  
...
![](images/attach-0ed50c402d935a07afa582abddccac598540e4eb.png)
...
![](images/attach-a045bb2cb6f786d0806cee6b63d925687d8c1fd1.png)
...
![](images/attach-fbe06e14f927e7d5ff250220eb9e3b23c7a60984.png)

c\_lookup方法要用了decodeObject方法，走到这里就想到在rmi中也要用了这个方法，并看到传入的参数var4其实跟rmi也是一样的，codebase、类名、工厂等。

![](images/attach-58418ad1e2330b3d87efa2dd2eeba8d2a84ce102.png)
...
![](images/attach-ecb18d3f5c9096000a5c88cf31546fd6da32320d.png)

![](images/attach-ca8e80752c8cd370eaa1d1e8abe48f8a99849bd5.png)
...
![](images/attach-62c0e549c1a95a0f19022701d45b776880ea1e3c.png)
...
效果图如下所示  
![](images/attach-4e6cfe21d46f63eb9e89d0fe9a3ed926361380f4.png)

调试跟踪分析流程

前面的流程跟RMI和LDAP是一样的，跟到RegistryContext.decodeObject()方法，工厂就是指定的org.apache.naming.factory.BeanFactory，  
![](images/attach-cac6c9cb6c6d8a6b3b82655e3ac0009196a59d6a.png)

接下来的流程也一样，走到getObjectFactoryFromReference方法，接着就是loadClass本地加载对应的类，clas不是null，就说明本地加载到了，

![](images/attach-036c295985b55eda169cf1d1af999f3ceb937624.png)

最后在getObjectInstance方法中反射的调用invoke执行EL表达式，完成命令执行。  
![](images/attach-baee8ebf077d87fa8984096ab08252566819a212.png)
...
![](images/attach-b84f24b761213fa4afd8f3aa6473492d7c1ec0ea.png)
...
![](images/attach-248a2cc752ede4cfe4f8925df262bdf44c710503.png)

deserializeObject方法中对var20进行反序列化，var20就是服务端在其中时给javaSerializedData的赋值，就是恶意的序列化数据。  
![](images/attach-4ba724ad953921f71d5e78bff7adfdbf07314d41.png)
...
![](images/attach-8658d98d122ed4194c914ce28c580a8ba15c3ed8.png)
